...
public abstract class SmartServer {
    ...
    public void log(Packet packet) {
        ...
        if(!Arrays.asList(505, 504).contains(packet.getType()))
            BotLogger.packet("↗ SEND " + packet.getType() + " " + packet.getClass().getSimpleName() + " [T:" + packet.getResponseTime() + "ms, P:" + JobService.getInstance().getThreadPool().getActiveCount() + "]");
    ...
    public void send(Go2Buffer buffer) {
        ...
        try {
            ...
            if(packetQueue.size() >= 100 && this instanceof GameServerReceiver) {
                GameServerReceiver serverReceiver = (GameServerReceiver) this;
