...
public class BionicChipListener implements PacketListener {
    ...
    public void onCommanderInsertCmos(RequestCommanderInsertCmosPacket packet) throws BadGuidException {
        ...
        if(packet.getCmosId() < 0) return;
        if(packet.getCmosType() != 0 && packet.getCmosType() != 1) return;
        if(!(packet.getHoleId() >= 0 && packet.getHoleId() <= 4)) return;
        ...
        if(packet.getCmosType() == 0) { // Insert
            ...
            if(commander.getChips().size() >= 5) return;
            if(packet.getCmosId() >= userChips.getChips().size()) return;

            int nextChipHole = commander.getNextChipHole();
            BionicChip selected = userChips.getChips().get(packet.getCmosId());

            if(selected == null) return;
            if(nextChipHole != packet.getHoleId()) return;
            ...
            for(BionicChip otherChip : commander.getChips()) {
                ...
                if(otherEffect == null) continue;

                if(otherEffect.eqauls(selectedEffect)) return;
            ...
            if(commanderChips.isEmpty()) return;
    ...
    public void onRequestGainCmosLottery(RequestGainCmostLotteryPacket packet) throws BadGuidException {
        ...
        if(packet.getPhaseId() < 0 || packet.getPhaseId() > 4) return;

        UserChips chips = user.getChips();
        if(packet.getPhaseId() != 0 && chips.getPhases()[packet.getPhaseId()] != 1) return;

        ChipData level = ResourceManager.getChips().getLevel(packet.getPhaseId() + 1);
        if(level == null) return;

        int slots = chips.getSlots();
        if(chips.getChips().size() + 1 > slots) return;

        UserResources resources = user.getResources();
        ChipMoneyMeta price = level.getPrice();

        boolean corsair = packet.getType() == 0 ? true : false;

        if(corsair && resources.getCorsairs() < price.getCorsair()) return;
        if(!corsair && resources.getMallPoints() < price.getMp()) return;

        int cost = corsair ? price.getCorsair() : price.getMp();

        if(corsair) resources.setCorsairs(resources.getCorsairs() - cost);
        else resources.setMallPoints(resources.getMallPoints() - cost);

        ChipRewardMeta reward = level.pickOne(corsair);
        if(reward == null) return;

        PropChipData chipData = reward.getChipData();
        boolean next = level.getNext() == -1 ? false : RandomUtil.getRandomInt(0, 100) <= level.getNext();

        int[] phases = chips.getPhases();
        if(next) phases[packet.getPhaseId() + 1] = 1;
        else phases[packet.getPhaseId()] = 0;
        ...
        response.setLotteryPhase((byte) lotteryPhase);
    ...
    public void onRequestSellProps(RequestSellPropsPacket packet) throws BadGuidException {
        ...
        if(packet.getType() == 1) {
            ...
            if(packet.getId() < 0 || packet.getId() >= chips.getChips().size()) return;
            if(chips.getChips().isEmpty()) return;
        ...
        if(prop == null) return;
        if(!user.getInventory().removeProp(prop, packet.getNum(), packet.getLockFlag() == 1 ? true : false)) return;
    ...
    public void onRequestUnionCmos(RequestUnionCmosPacket packet) throws BadGuidException {
        ...
        if(packet.getCmosId1() == packet.getCmosId2()) return;
        if(packet.getCmosId1() < 0 || packet.getCmosId1() >= chips.getChips().size()) return;
        if(packet.getCmosId2() < 0 || packet.getCmosId2() >= chips.getChips().size()) return;
        if(chips.getChips().isEmpty()) return;
    ...
    public void onRequestOpenCmos(RequestOpenCmosPack packet) throws BadGuidException {
        ...
        if(chips.getSlots() + 1 > 30) return;

        int price = (chips.getSlots() - 15) + 1;
        if(resources.getMallPoints() < price) return;
    ...
    private byte getLotteryPhase(UserChips chips) {
        ...
        if(chips.getPhases()[1] == 1) lotteryPhase |= (1 << 0);
        if(chips.getPhases()[2] == 1) lotteryPhase |= (1 << 1);
        if(chips.getPhases()[3] == 1) lotteryPhase |= (1 << 2);
        if(chips.getPhases()[4] == 1) lotteryPhase |= (1 << 3);
