...
public class FleetListener implements PacketListener {
    ...
    public void onViewJumpShipTeam(RequestViewJumpShipTeamPacket packet) throws BadGuidException {
        ...
        response.setAim((short) commander.getTotalAim(baseAttributes));
        response.setBlench((short) commander.getTotalDodge(baseAttributes));
        response.setPriority((short) commander.getTotalSpeed(baseAttributes));
        response.setElectron((short) commander.getTotalElectron(baseAttributes));
        response.setSkillId((short) commander.getSkill());
    ...
    public void onCancelJumpShipTeam(RequestCancelJumpShipTeamPacket packet) throws BadGuidException {
        ...
        Fleet fleet = PacketService.getInstance().getFleetCache().findByShipTeamId(packet.getShipTeamId());
        if (fleet == null) return;
        if (fleet.getGuid() != user.getGuid()) return;
        if (fleet.isInMatch() || !fleet.isInTransmission()) return;
        ...
        if (transmission.getGalaxyId() == userGalaxyId || transmission.getJumpType() == JumpType.RECALL) return;
    ...
    public void onJumpShipTeam(RequestJumpShipTeamPacket packet) throws BadGuidException {
        ...
        if (packet.getDataLen() == 0 || packet.getShipTeamId().getArray().length <= packet.getDataLen()) return;
        ...
        switch (planet.getType()) {
            ...
            case USER_PLANET -> {
                ...
                if (targetCorp != null && targetCorp.getName() != null && targetCorp.getName().equals(CorpService.COUNCIL_CORP_NAME)) {
                    ...
                    packet.getSmartServer().sendMessage("Commander, we can't attack the council!");
                    return;
                ...
                if (attack) {
                    ...
                    if (userTarget.getStats().hasTruce()) return;
            ...
            case HUMAROID_PLANET -> {
                ...
                if (!attack) return;
                if (humaroidPlanet.hasTruce() || humaroidPlanet.isInWar()) return;
            ...
            case RESOURCES_PLANET -> {
                ...
                if (!attack) return;
                if (resourcePlanet.hasTruce()) return;
        ...
        if (sync) {
            ...
            for (Fleet fleet : formation) {
                ...
                double currentTransmission = fleet.getTransmissionRate() * currentDistance + fleet.getTransmissionStart();
        ...
        if (corpNotice.isPresent()) {
            ...
            for (CorpMember corpMember : corp.getMembers().getMembers()) {
                ...
                if (loggedGameUser.isPresent()) loggedGameUser.get().getSmartServer().send(noticePacket);
            }
    ...
    public void onJumpShipTeamInfo(RequestJumpShipTeamInfoPacket packet) throws BadGuidException {
        ...
        for (Fleet fleet : filtered) {
            ...
            if (transmission != null) {
                ...
                if (fleet.getGuid() == user.getGuid()) {
                    ...
                    if (transmission.getJumpType() == JumpType.ATTACK) kind = 1;
                    else kind = 2;
        ...
        if (currentResponse != null) responsePackets.add(currentResponse);
    ...
    public void onJumpGalaxyShip(RequestJumpGalaxyShipPacket packet) throws BadGuidException {
        ...
        switch (packet.getKind()) {
            ...
            case 0 -> { // Planet
                ...
                switch (planet.getType()) {
                    ...
                    case USER_PLANET:
                        ...
                        if (optionalUserTarget.isEmpty()) return;
                        ...
                        if (userTarget.getStats().hasTruce()) return;
                        if (userPlanet.isInWar()) jumpType = 2;
                        else jumpType = 1;
                        break;
                    ...
                    case HUMAROID_PLANET:
                        ...
                        HumaroidPlanet humaroidPlanet = (HumaroidPlanet) planet;
                        if (humaroidPlanet.hasTruce()) return;
            ...
            case 1 -> { // RBP Planet
                ...
                if (jumpCorp == null) return;

                Planet planet = PacketService.getInstance().getPlanetCache().findByPosition(targetTile);
                if (planet.getType() != PlanetType.RESOURCES_PLANET) return;

                ResourcePlanet resourcePlanet = (ResourcePlanet) planet;
                Optional<Corp> optionalCorp = resourcePlanet.getCorp();
                if (optionalCorp.isEmpty()) return;

                Corp corp = optionalCorp.get();
                if (corp.getCorpId() != jumpCorp.getCorpId()) return;
    ...
    public void onEditShipTeam(RequestEditShipTeamPacket packet) throws BadGuidException {
        ...
        User user = UserService.getInstance().getUserCache().findByGuid(packet.getGuid());
        if (user == null || packet.getShipTeamId() < 0) return;

        Commander commander = user.getCommander(packet.getCommanderId());
        Fleet ccFleet = commander.getFleet();
        if (commander == null || (ccFleet != null && ccFleet.getShipTeamId() != packet.getShipTeamId())) return;

        Fleet fleet = PacketService.getInstance()
                .getFleetCache()
                .findByShipTeamId(packet.getShipTeamId());

        if(fleet == null || fleet.getShipTeamId() != packet.getShipTeamId()) return;
        if(fleet.isMatch() || fleet.isInMatch() || fleet.isInTransmission()) return;
        ...
        for (int idx = 0; idx < 9; ++idx) {
            ...
            System.out.println(newNum.getShipModelId() + " " + newNum.getNum());
        ...
        // All clear, finish up
        // First, return old fleet to inventory
        ...
        // Finally, apply the rest of the changes & save
    ...
    public void onCreateFleet(RequestCreateShipTeamPacket packet) throws BadGuidException {
        ...
        Commander commander = user.getCommander(packet.getCommanderId());
        if (commander == null) return;
        if (PacketService.getInstance().getFleetCache().findByCommanderId(commander.getCommanderId()) != null) return;

        UserPlanet userPlanet = user.getPlanet();
        if (userPlanet.isInWar()) return;
        ...
        if (optionalGameUser.isEmpty()) return;
    ...
    public void onArrangement(RequestArrangeShipTeamPacket packet) throws BadGuidException {
        ...
        List<TeamModel> models = new ArrayList<>(VariableType.MAX_TEAMMODEL);

        for (int i = 0; i < 3; i++) {
            if (i < teamModelSlots.size()) {
                TeamModelSlot models1 = teamModelSlots.get(i);
                models.add(models1.getTeamModel());
            } else {
                TeamModel teamModel = new TeamModel();
                models.add(teamModel);
            }
        }
    ...
    public void onMoveShipTeam(RequestMoveShipTeamPacket packet) throws BadGuidException {
        ...
        if (packet.getPosX() < 0 || packet.getPosY() < 0) return;
        if (packet.getPosX() > 24 || packet.getPosY() > 24) return;

        Fleet fleet = PacketService.getInstance().getFleetCache().findByShipTeamId(packet.getShipTeamId());

        if (fleet == null || fleet.getGuid() != packet.getGuid() || fleet.getGalaxyId() == -1) return;
        if (fleet.isMatch() || fleet.isInMatch() || fleet.isInTransmission()) return;
    ...
    public void onDirectionShipTeam(RequestDirectionShipTeamPacket packet) throws BadGuidException {
        ...
        if (user == null) return;
        if (packet.getDirection() < 0 || packet.getDirection() > 3) return;
