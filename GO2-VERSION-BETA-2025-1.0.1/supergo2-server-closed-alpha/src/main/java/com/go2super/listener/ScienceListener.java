...
public class ScienceListener implements PacketListener {
    ...
    public void onUnbind(RequestUnbindCommanderCardPacket packet) throws BadGuidException {
        ...
        if(userResources.getMallPoints() < 100) return;
        if(!userInventory.hasProp(packet.getPropsId(), 1, 0, true)) return;
    ...
    public void onCreateTech(RequestCreateTechPacket packet) throws BadGuidException {
        ...
        if(userCorp != null) scienceBonus += userCorp.getRBPBonus();
        ...
        if(!researchData.meetRequirements(user) || user.getTechs().getUpgrade() != null) return;
        if(research != null && researchData.getLevels().size() < research.getLevel() + 1) return;
        ...
        if(user.getResources().getGold() < levelMeta.getGold()) return;
    ...
    public void onCancelTech(RequestCancelTechPacket packet) throws BadGuidException {
        ...
        if(userTechs.getUpgrade() == null) return;
