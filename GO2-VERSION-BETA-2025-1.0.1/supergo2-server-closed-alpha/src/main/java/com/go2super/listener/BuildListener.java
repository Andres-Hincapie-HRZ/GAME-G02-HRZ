...
public class BuildListener implements PacketListener {
    ...
    public void onBuildMove(RequestMoveBuildPacket packet) throws BadGuidException {
        ...
        if(user == null || packet.getIndexId() < 0) return;

        UserBuilding building = user.getBuildings().getBuilding(packet.getIndexId());
        if(building == null) return;
        if(isInvalid(building.getData(), packet.getPosX().getValue(), packet.getPosY().getValue())) return;
    ...
    public void onBuild(RequestCreateBuildPacket packet) throws BadGuidException {
        ...
        if(data == null) return;
        if(isInvalid(data, packet.getPosX().getValue(), packet.getPosY().getValue())) return;
        ...
        if(user.getStats().hasBonus(BonusType.CONSTRUCTION_SLOTS)) slots += 3;
        if(buildings.getUpdatingBuildings().size() + 1 > slots) return;
        ...
        if(building == null) {
                ...
                for (BuildIfMeta ifMeta : data.getLimitIf()) {
                    switch (ifMeta.getType()) {
                        ...
                        case "building":
                            ...
                            bloop:for (UserBuilding build : builds)
            ...
            if(boostTech != null) time = (int) ((double) time / (1 + (boostTech.getEffectValue("increase.construction.speed") * 0.01)));
            ...
            UserTech boostTech = techs.getTech("science:construction.boost");
            if(boostTech != null) time -= time * (boostTech.getEffectValue("increase.construction.speed") * 0.01);
    ...
    public void onCancel(RequestCancelBuildPacket packet) throws BadGuidException {
        ...
        if(building == null) return;
        if(building.getUpdating() == null) return;
        if(!building.getUpdating()) return;
    ...
    public void onConsortiaBuilding(RequestConsortiaBuildingPacket packet) throws BadGuidException {
        ...
        if(userCorp == null || userCorp.getMembers().getLeader().getGuid() != user.getGuid()) return;

        Optional<Corp> planetCorp = resourcePlanet.getCorp();
        if(!planetCorp.isPresent() || planetCorp.get().getCorpId() != userCorp.getCorpId()) return;

        RBPBuildings rbpBuildings = resourcePlanet.getBuildings();
        if(rbpBuildings == null || rbpBuildings.getBuildings().isEmpty()) return;

        RBPBuilding rbpBuilding = rbpBuildings.getBuilding(packet.getIndexId());
        if(rbpBuilding == null) return;

        FortificationData fortificationData = rbpBuilding.getData();
        if(fortificationData == null) return;
        ...
        if(fortificationLevelMeta == null || nextFortificationLevelMeta == null) return;

        Optional<Integer> optionalWealthRequired = nextFortificationLevelMeta.getWealthRequired();
        if(optionalWealthRequired.isEmpty()) return;

        int wealthRequired = optionalWealthRequired.get();
        if(userCorp.getWealth() < wealthRequired) return;
    ...
    public boolean isInvalid(BuildData buildData, int x, int y) {
        ...
        if(buildData.getType().equals("space")) {
            if(x > SPACE_BOUND.getMaximumX() || x < SPACE_BOUND.getMinimumX() ||
                ...
                return true;
            if(SPACE_INVALID.isInvalid(x, y))
                return true;
        } else {
