...
public class PropListener implements PacketListener {
    ...
    public void useProp(RequestUsePropsPacket packet) {
        ...
        if(prop == null) return;
        if(!user.getInventory().hasProp(prop.getPropId(), packet.getNum(), 0, lock)) return;
    ...
    public void deleteProp(RequestDeletePropsPacket packet) throws BadGuidException {
        ...
        if(prop == null) return;
        if(!user.getInventory().removeProp(prop, 1, packet.getLockFlag() == 1 ? true : false)) return;
        ...
        buffer.append("**Item:** `" + getItemName(prop) + " (Amount: " + 1 + ", Bound: " + (packet.getLockFlag() == 1 ? "Yes" : "No") + ")`");

        DiscordService.getInstance().getRayoBot().sendAudit("Inventory Item Delete", buffer.toString(), Color.decode("0xe3ac3d"), AuditType.DELETE);
    ...
    public void onMoveProp(RequestMovePropPacket packet) throws BadGuidException {
        ...
        LoginService.validate(packet, packet.getGuid());
        if(packet.getPropNum() <= 0 || packet.getPropId() < 0) return;
        ...
        boolean lock = packet.getLockFlag() == 1 ? true : false;
        boolean toCorp = packet.getKind() == 0 ? true : false;
        ...
        if(lock && toNum > prop.getPropLockNum()) return;
        if(!lock && toNum > prop.getPropNum()) return;

        if(toCorp) {

            if(!user.getCorpInventory().addProp(prop.getPropId(), toNum, 1, lock)) return;
            if(!user.getInventory().removeProp(prop, toNum, lock)) return;

        } else {

            if(!user.getInventory().addProp(prop.getPropId(), toNum, 0, lock)) return;
            if(!user.getCorpInventory().removeProp(prop, toNum, lock)) return;
