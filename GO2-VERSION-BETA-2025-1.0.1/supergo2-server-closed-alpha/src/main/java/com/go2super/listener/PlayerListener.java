...
public class PlayerListener implements PacketListener {
    ...
    public void onRequestPlayerInfo(RequestPlayerInfoPacket packet) {
        ...
        if(serverReceiver.getGuid() != packet.getGuid()) return;
     ...
     //   packet.reply(getTaskInfoPacket(user));
        ...
        if(roleInfoPacket != null) packet.reply(roleInfoPacket);
        ...
        if(!user.getStats().getBuffs().isEmpty()) packet.reply(user.getQueuesAsPacket());
        ...
        if(onlineAwardPacket != null) packet.reply(onlineAwardPacket);
    ...
    public void onFriendInfo(RequestFriendInfoPacket packet) {
        ...
        if(packet.getObjGuid() >= 0) user = UserService.getInstance().getUserCache().findByGuid(packet.getObjGuid());
        else if(packet.getObjUserId() >= 0) user = UserService.getInstance().getUserCache().findByUserId(packet.getObjUserId());
    ...
    private ResponseUserInfoPacket getHumaroidUserInfo(Planet planet) {
        ...
        if(humaPlanet.getStatusTime() != null) {
            ...
            if(humaPlanet.isDestroyed()) packet.setPeaceTime(-DateUtil.remains(humaPlanet.getStatusTime()).intValue());
            else packet.setPeaceTime(DateUtil.remains(humaPlanet.getStatusTime()).intValue());
    ...
    private ResponseUserInfoPacket getRBPUserInfo(Planet planet) {
        ...
        if(resourcePlanet.getStatusTime() != null) {
            ...
            if(resourcePlanet.isPeace()) packet.setPeaceTime(DateUtil.remains(resourcePlanet.getStatusTime()).intValue());
            else packet.setPeaceTime(-DateUtil.remains(resourcePlanet.getStatusTime()).intValue());
    ...
    private ResponseShipBodyInfoPacket getShipBodyInfoPacket(User user) {
        ...
        if(shipUpgrades.getCurrentBodies() == null) shipUpgrades.setCurrentBodies(new ArrayList<>());
        if(shipUpgrades.getCurrentParts() == null) shipUpgrades.setCurrentParts(new ArrayList<>());
    ...
    private ResponseShipBodyUpgradeInfoPacket getShipBodyUpgradeInfoPacket(User user) {
        ...
        if(shipUpgrades.getShipUpgrade() != null) {
            ...
            response.setBodyId(Arrays.asList(shipBodyInfo));

        } else {

            response.setBodyNum((short) 0);
            response.setBodyId(Arrays.asList(ShipBodyInfo.builder()
                    .bodyPartId(-1)
                    .needTime(-1)
                    .build()));
        ...
        if(shipUpgrades.getPartUpgrade() != null) {
            ...
            response.setPartId(Arrays.asList(shipBodyInfo));

        } else {

            response.setPartNum((short) 0);
            response.setPartId(Arrays.asList(ShipBodyInfo.builder()
                    .bodyPartId(-1)
                    .needTime(-1)
                    .build()));
