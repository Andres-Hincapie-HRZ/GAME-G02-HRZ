...
public class MallListener implements PacketListener {
    ...
    public void onBuyGoods(RequestBuyGoodsPacket packet) throws BadGuidException {
        ...
        if(packet.getNum() <= 0 || packet.getNum() > 9999) return;
        ...
        Optional<PropData> optionalGood = props.stream()
            .filter(cache -> {
                String name = cache.getName();
                if(name != null && name.toLowerCase().contains("testing pack")) return false;
                return cache.getId() == packet.getPropsId();
            })
            .findAny();
        if(!optionalGood.isPresent()) return;
        ...
        if(!optionalMall.isPresent()) return;
        ...
        if(good.getId() == 921) quantity = packet.getNum() * mall.getAmount() * 10;
        else quantity = packet.getNum() * mall.getAmount();
        ...
        if(good.getType().equals("chip")) {
            ...
            if(packet.getNum() > 30) return;
            if((chips.getChips().size() + packet.getNum()) > slots) return;
        ...
        switch(mall.currencyCode()) {
            ...
            case 0: // MP
                ...
                if(resources.getMallPoints() >= price) resources.setMallPoints(user.getResources().getMallPoints() - price);
                else return;
            ...
            case 1: // VOUCHERS
                ...
                if(resources.getVouchers() >= price) resources.setVouchers(user.getResources().getVouchers() - price);
                else return;
            ...
            case 2: // BADGE
                ...
                if(resources.getBadge() >= price) resources.setBadge(user.getResources().getBadge() - price);
                else return;
            ...
            case 3: // HONOR
                ...
                if(resources.getHonor() >= price) resources.setHonor(user.getResources().getHonor() - price);
                else return;
            ...
            case 4: // CHAMPS
                ...
                if(resources.getChampionPoints() >= price) resources.setChampionPoints(user.getResources().getChampionPoints() - price);
                else return;
        ...
        } else if(!user.getInventory().addProp(good.getId(), quantity, 0, mall.isBound())) return;
