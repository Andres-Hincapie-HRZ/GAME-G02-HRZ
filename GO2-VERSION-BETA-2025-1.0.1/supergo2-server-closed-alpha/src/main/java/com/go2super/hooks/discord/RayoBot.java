...
public class RayoBot extends DiscordBot {
    ...
    private static Set<RayoPackage> rayoPackages = new HashSet<>();
    ...
    public void start(String discordToken) {
        ...
        this.jda = JDABuilder.createLight(discordToken, Collections.emptyList())
                ...
                .setActivity(Activity.playing("SuperGO2"))
                .build().awaitReady();
    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        ...
        if(event.getMessageChannel() == null) return;
        ...
        if(event.getName().equals("user-ignore")) {
            ...
            if(!(riskIncident instanceof SameIPIncident)) {
            ...
            SameIPIncident sameIPIncident = (SameIPIncident) riskIncident;
            ...
            if(optionalSameIPIncidentInfo.isPresent()) {
                ...
                if(user.getIgnore() == null) user.setIgnore(true);
                else user.setIgnore(!user.getIgnore());
        ...
        if(event.getName().equals("online")) {
            ...
            if(players.isEmpty()) eb.setDescription("No players online!");
            else eb.setDescription(StringUtils.join(players, ", "));
        ...
        if(event.getName().equals("performance")) {
            ...
            String info =
                    ...
                    "**Queue** = `" + JobService.getInstance().getThreadPool().getActiveCount() + "`\n" +
                    "**Peak** = `" + maxPeak() + "`\n" +
        ...
        if(event.getName().equals("bloc")) {
            ...
            if(corp.getBlocId() == null || corp.getBlocId().isEmpty()) {
            ...
            List<Corp> corps = CorpService.getInstance().getCorpCache().findByBlocId(bloc.getId().toString());
            ...
            if(corps.size() > 1) names = StringUtils.join(cnames, ", ");
            else names = corps.get(0).getName() + " (ID: " + corps.get(0).getCorpId() + ")";
        ...
        if(event.getName().equals("create-bloc")) {
            ...
            corp.setBlocId(bloc.getId().toString());
            corp.save();
