...
// Anotación que indica que esta clase es un documento MongoDB en la colección "game_account_sessions"
...
// Clase que representa una sesión de cuenta de usuario
public class AccountSession {

    // Campo ID único de MongoDB
    @Id // Anotación JPA para ID
    private ObjectId id; // ID único de la sesión

    // Campo transitorio que referencia la cuenta, no se guarda en DB
    @Transient // No se persiste en la base de datos
    private Account reference; // Referencia a la cuenta asociada

    // Campos únicos para ID de cuenta y token de sesión
    @Column(unique = true) private String accountId; // ID de la cuenta asociada
    ...
    public AccountSession reference(Account account) { // Establece referencia a cuenta
