...
public class PacketService {
    ...
    @Getter private ShipModelCache shipModelCache;
    @Getter private PlanetCache planetCache;
    @Getter private SanctionCache sanctionCache;
    @Getter private FleetCache fleetCache;
    ...
    @Value("${application.game.custom-instances}")
    @Getter
    private boolean customInstances;
    ...
    public boolean hasFleetsDeployed(User user) {
        ...
        for(Fleet fleet : fleets)
            if(fleet.isInTransmission() || (fleet.getGalaxyId() != user.getPlanet().getPosition().galaxyId())) {
                ...
                if(match != null && match.getMatchType().isVirtual()) continue;
                return true;
            }
    ...
    public List<Packet> getShipModels(User user, Set<Integer> shipModelIds) {
        ...
        for(int modelId : shipModelIds) {
            ...
            if(model == null) {
                BotLogger.error("ShipModel = " + modelId + " NOT FOUND! ");
                continue;
            }
        ...
        List<Pair<ShipModelInfo, Boolean>> delModels = toSend.stream().filter(pair -> pair.getValue() == true).collect(Collectors.toList());
        List<Pair<ShipModelInfo, Boolean>> commonModels = toSend.stream().filter(pair -> pair.getValue() == false).collect(Collectors.toList());
    ...
    public boolean sendMoreInfoPacket(int kind, User user, Packet from) {
        ...
        if(smartServer.getSentMoreInfos().contains(user.getUserId())) return false;
    ...
    public boolean sendMoreInfoPacket(int kind, long userId, Packet from) {
        ...
        SmartServer smartServer = from.getSmartServer();
        if(smartServer.getSentMoreInfos().contains(userId)) return false;
    ...
    public CustomMoreInfoPacket getMoreInfoPacket(int kind, long userId) {
        ...
        customMoreInfoPacket.setIcon(user.getIcon());
