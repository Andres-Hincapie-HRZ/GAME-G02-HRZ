...
public class CommanderService {
    ...
    @Getter private CommanderCache commanderCache;
    ...
    public ResponseRefreshCommanderBaseInfoPacket getRefreshBaseInfoPacket(User user) {
        ...
        for (Commander commander : commanders) {
            ...
            if(packet.getCommanderBaseInfos().size() == 60) break;
        ...
        for(Commander commander : commanders)
            if(commander.getCommanderId() == commanderId) {
                ...
                if(fleet != null) {
                    ...
                    response.setTarget((byte) fleet.getRangeType());
                    response.setTargetInterval((byte) fleet.getPreferenceType());
    ...
    public CommanderTrigger createTrigger(BattleCommander battleCommander) {
        ...
        if(battleCommander.getSkillId() == -1) return new CommanderTrigger();
        ...
        for(String stat : procMeta.getStat()) {
            switch (stat) {
                case "accuracy": integral += battleCommander.getTotalAccuracy(); break;
                case "dodge": integral += battleCommander.getTotalDodge(); break;
                case "speed": integral += battleCommander.getTotalSpeed(); break;
                case "electron": integral += battleCommander.getTotalElectron(); break;
            }
        }
    ...
    public CommanderTrigger createTrigger(BattleCommander battleCommander, BattleCommander copiedCommander) {
        ...
        if(statsData == null) return new CommanderTrigger();
    ...
    public Commander basic(int skill, int stars, int experience, long userId) {
        ...
        if(statsData == null) return common(userId);
    ...
    public CommanderPullulateData getCommanderPullulate(Commander commander) {
        return getCommanderPullulate(commander.isCommon() ? 0 : commander.getStats().typeCode(), commander.getStars());
    }
