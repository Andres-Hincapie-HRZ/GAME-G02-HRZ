...
public class DashboardService {
    ...
    public static int MAX_USER = 1;

    String usernamePattern = "^[\\w\\d\\S]{1,32}$";
    Pattern pattern = Pattern.compile(usernamePattern);

    private static DashboardService accountService;
    ...
    @Getter private PlanetCache planetCache;

    @Autowired
    public DashboardService(UserCache userCache, AccountCache accountCache, PlanetCache planetCache) {
    ...
    public BasicResponse play(long userId, HttpServletRequest request){
        ...
        Optional<User> optionalUser = users.stream().filter(x -> x.getUserId() == userId).findAny();
    ...
    public BasicResponse createUser(CreateUserDTO dto, HttpServletRequest request) {
        ...
        if(dto.getGround() > 3 || dto.getGround() < 1)
            return BasicResponse
                    .builder()
                    .code(HttpStatus.OK.value())
        ...
        newUser.setUserEmailStorage(createNewUserEmailStorage());
        ...
        if(rankJob != null) rankJob.synchronizedAdd(newUser);
    ...
    public UserShipUpgrades createNewUserShipUpgrades() {
        return UserShipUpgrades.builder()
                .currentBodies(new ArrayList<>(Arrays.asList(0)))
    ...
    public UserEmailStorage createNewUserEmailStorage() {

        UserEmailStorage userEmailStorage =  UserEmailStorage.builder()
                .userEmails(new ArrayList<>())
                .build();

        createWelcomeEmail(userEmailStorage);
        return userEmailStorage;

    }

    public void createWelcomeEmail(UserEmailStorage userEmailStorage) {

        Email email = Email.builder()
                .autoId(userEmailStorage.nextAutoId())
                .type(2)
                .name("System")
                .subject("Welcome to SuperGO2!")
                .emailContent(
                        "Welcome to SuperGO2, Commander!" +
                        "" +
                        "  The Intergalactic Council is thrilled to have you join us in this futuristic era of universal warfare, where you can amass wealth and engage in epic battles against other Commanders and Corps. To help you get on your way, please refer to the Quest system each day so that you can fulfill your duties as a member of this exciting and dangerous new world. We look forward to great achievements from you! Here are some supplies to get you started on the right foot!")
                .readFlag(0)
                .date(DateUtil.now())
                .goods(new ArrayList<>())
                .guid(-1)
