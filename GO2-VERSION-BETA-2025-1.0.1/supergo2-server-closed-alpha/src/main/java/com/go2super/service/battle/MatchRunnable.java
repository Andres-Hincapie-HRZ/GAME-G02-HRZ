...
public class MatchRunnable implements Runnable {
    ...
    public void step() {
        ...
        if(new Date().getTime() <= actionFinishDate)
            return;
    ...
    public void calculateNextFightFortressSection(BattleAction battleAction) {
        ...
        for(BattleActionTrigger trigger : battleAction.getTriggers()) {
            ...
            if(trigger instanceof FortAttackFleetTrigger) {

                FortAttackFleetTrigger attackTrigger = (FortAttackFleetTrigger) trigger;
                ...
                for(FortCellAttackMeta attackMeta : attackTrigger.getAttacks()) {
                    ...
                    if(attackMeta == null || !attackMeta.isAttack()) continue;
                    ...
                    if(targetDestroyed) fortressFight.setDelFlag((byte) 1);

                }

                for(Integer targetFleetId : targetFleets) {

                    BattleFleet targetFleet = getCurrentFleet(targetFleetId);
                    if(targetFleet == null) continue;

                    List<ResponseFightInitShipTeamPacket> shipTeamPackets = BattleService.getInstance().getFightInitShipTeamPackets(match, targetFleet);
                    match.sendPacketToViewers(shipTeamPackets.stream().toArray(ResponseFightInitShipTeamPacket[]::new));

                    targetFleet.update(match);
        ...
        if(fightFortressSectionPacket != null) fightFortressPackets.add(fightFortressSectionPacket);
    ...
    public void calculateNextFightSection(ResponseFightSectionPacket fightSectionPacket, BattleAction battleAction) {
        ...
        for(BattleActionTrigger trigger : battleAction.getTriggers()) {
            ...
            if(trigger instanceof MovementTrigger) {
                ...
                MovementTrigger movementTrigger = (MovementTrigger) trigger;
            ...
            if(trigger instanceof FleetRotateTrigger) {

                FleetRotateTrigger rotateTrigger = (FleetRotateTrigger) trigger;
            ...
            if(trigger instanceof FleetAttackFleetTrigger) {

                fightSectionPacket.setShipFights(new ArrayList<>());

                FleetAttackFleetTrigger attackTrigger = (FleetAttackFleetTrigger) trigger;

                fightSectionPacket.setSourceSkill(UnsignedChar.of(attackTrigger.isSourceAnimation() ? 1 : 0));
                ...
                for(FleetEffects fleetEffects : attackTrigger.getFleetEffects()) {
                    ...
                    if(targetFleet.isDestroyed()) continue;
                ...
                for(int i = 0; i < 9; i++) {
                    ...
                    for(ModuleUsage moduleUsage : attackMeta.getDefenderUsages()) {
                        ...
                        if(defenderCell.isEmpty()) continue;
                    ...
                    if(attackMeta.getAttackerHighestAttack() != null) match.getBattleReport().processHighestAttack(attackMeta.getAttackerHighestAttack());
                    if(attackMeta.getDefenderHighestAttack() != null) match.getBattleReport().processHighestAttack(attackMeta.getDefenderHighestAttack());
                ...
                if(targetDestroyed) match.removeFleet(targetBattleFleet);
                if(sourceDestroyed) match.removeFleet(battleFleet);
            ...
            if(trigger instanceof FleetAttackFortTrigger) {

                fightSectionPacket.setBothStatus((byte) 1);
                fightSectionPacket.setShipFights(new ArrayList<>());

                FleetAttackFortTrigger attackTrigger = (FleetAttackFortTrigger) trigger;
    ...
    public void calculateNextRound() {
        ...
        if(!match.getMatchType().isVirtual()) {
            ...
            if(!start.isEmpty()) {
                ...
                if(!enemies.isEmpty()) match.getFleets().addAll(createdEnemies);
                if(!allies.isEmpty()) match.getFleets().addAll(createdAllies);
        ...
        for(BattleFleet battleFleet : match.getFleets()) {
            ...
            if(battleFleet.isDestroyed()) continue;
            ...
            // ! Aileen [Skill] #2
            if(effects.contains(EffectType.AILEEN)) {
            ...
            // ! Bain [Skill] #2
            if(effects.contains(EffectType.BAIN)) {
            ...
            for(BattleFleetCell cell : battleFleet.getCells()) {
                ...
                for(BattleFleetDefensiveModule defensiveModule : regenModules) {
                    ...
                    if(battleFleet.getHe3() < (defensiveModule.getFuelUsage() * effectiveStack)) continue;
                    ...
                    double shieldHeal = defensiveModule.getShieldHeal() * (1 + commander.getShieldHealIncrement());
        ...
        for(BattleFleet fleet : temporalFleets) {
            ...
            calculateAction(fleet, temporalElements);
    ...
    public void calculateAction(BattleFleet battleFleet, List<BattleElement> temporalElements) {
        ...
        // Commander Skills
        {
            ...
            if(battleFleet.isCommanded("commander:krinaKlaus")) {
                if(battleFleet.trigger("commander:krinaKlaus")) {
                    battleFleet.setAnimation(true);
                    for(BattleFleet other : match.getFleets()) {
                        if(other == battleFleet) continue;
                        if(other.isEnemy(battleFleet)) continue;
            ...
            if(battleFleet.isCommanded("commander:ironMaidens")) {
                if(battleFleet.trigger("commander:ironMaidens")) {
                    ...
                    for(BattleFleetCell fleetCell : battleFleet.getCells()) {
                        fleetCell.setShields(Math.max(fleetCell.getMaxShields(), 1));
                        fleetCell.setStructure(Math.max(fleetCell.getMaxStructure(), 1));
                    }
                }
            }
            ...
            if(battleFleet.isCommanded("commander:enduringChorus")) {
                if(battleFleet.trigger("commander:enduringChorus")) {
                    ...
                    for(BattleFleet other : match.getFleets()) {
                        if(other == battleFleet) continue;
                        if(other.isEnemy(battleFleet)) continue;
                        if(!battleFleet.canAffect(other)) continue;
                        if(other.getGO2Node().getHeuristic(battleFleet.getGO2Node()) <= 4) {
                            other.getBattleCommander().setTotalDodge(other.getBattleCommander().getTotalDodge() + 5);
                        }
                    }
                }
            }
            ...
            if(battleFleet.isCommanded("commander:leechLurkers")) {
                if(battleFleet.trigger("commander:leechLurkers")) {
                    ...
                    FleetEffect effect = FleetEffect.builder()
                            ...
                            .value(-1)
                            .until(match.getRound() + 2)
                            .build();
                    battleFleet.process(effect);

                    double reduction = 0.05d;
                    battleFleet.setAnimation(true);
                    for(BattleFleet other : match.getFleets()) {
                        if(other == battleFleet) continue;
                        if(!other.isEnemy(battleFleet)) continue;
                ...
                if(battleFleet.trigger("commander:lurkingLight")) {
                    ...
                    for(BattleFleet other : match.getFleets()) {
                        ...
                        if(other.getGO2Node().getHeuristic(battleFleet.getGO2Node()) <= 4) {
                            for(BattleFleetCell cell : other.getCells()) {
                                if(!cell.hasShips()) continue;
                                cell.setShields((int) Math.max(cell.getShields() + (cell.getShields() * addition), 0));
                                cell.setStructure((int) Math.max(cell.getStructure() + (cell.getStructure() * addition), 0));
                            }
                        }
                    }
            ...
            if(battleFleet.isCommanded("commander:queensOfBlades")) {
                if(true || battleFleet.trigger("commander:queensOfBlades")) {
                    battleFleet.setAnimation(true);
                    FleetEffect effect = FleetEffect.builder()
                            .effectType(EffectType.BART)
                            .value(-1)
            ...
            if(battleFleet.isCommanded("commander:kelly")) {
                if(battleFleet.trigger("commander:kelly")) {
                    battleFleet.setAnimation(true);
                    for(BattleFleetCell fleetCell : battleFleet.getCells())
                        fleetCell.setStructure((int) Math.min(fleetCell.getStructure() * 1.15, fleetCell.getMaxStructure()));
                }
            }
            ...
            if(battleFleet.isCommanded("commander:donna")) {
                if(battleFleet.trigger("commander:donna")) {
                    battleFleet.setAnimation(true);
                    for(BattleFleetCell fleetCell : battleFleet.getCells())
                        fleetCell.setShields((int) Math.min(fleetCell.getShields() * 1.15, fleetCell.getMaxShields()));
                }
            }
        ...
        if(target != null) BotLogger.dev(battleFleet.getGuid() + " (" + battleFleet.getBattleCommander().getNameId() + ") meantime is: " + target.getNode());
        else BotLogger.dev(battleFleet.getGuid() + " (" + battleFleet.getBattleCommander().getNameId() + ") meantime is: null");
        ...
        if(path != null) BotLogger.dev(battleFleet.getGuid() + " (" + battleFleet.getBattleCommander().getNameId() + ") steps are: " + steps + ", turns are: " + path.turns());
        else BotLogger.dev(battleFleet.getGuid() + " (" + battleFleet.getBattleCommander().getNameId() + ") steps & turns are: 0");
            ...
            switch(target.getType()) {
                ...
                case FLEET -> {
                    ...
                    if(successAttack) {
                        ...
                        attackTrigger.setSourceAnimation(battleFleet.isAnimation());
    ...
    public void calculateAction(BattleFort battleFort, List<BattleElement> temporalElements) {
        ...
        if(battleFort.isDestroyed() || !battleFort.hasCannon()) return;
        ...
        List<BattleFleet> attackable = temporalElements.stream()
                ...
                .filter(fleet -> !fleet.isDestroyed() && match.fortressAttackType().isAttacker() == fleet.isAttacker())
                .collect(Collectors.toList());
        ...
        switch(battleFort.getAttackType()) {
            ...
            case ALL_ATTACK -> {
                ...
                for(BattleFleet target : attackable) {
                    ...
                    if(distance <= battleFort.getRange()) reachedTargets.add(target);
                }
            ...
            case RADIUS_TARGET -> {
                ...
                reach : for(BattleFleet target : attackable) {
                    int distance = target.getGO2Node().getHeuristic(battleFort.getGO2Node());
                ...
                scatter : for(BattleFleet target : attackable) {
                    int distance = target.getGO2Node().getHeuristic(reached.getGO2Node());
                    if(distance <= 1) reachedTargets.add(target);
                }
            ...
            case SINGLE_TARGET -> {
                ...
                if(reached != null) targets = Arrays.asList(reached);
                break;
    ...
    public BattleElement calculateTarget(BattleFleet battleFleet, List<BattleElement> temporalElements) {
        ...
        for(BattleElement element : temporalElements)
            if(element.getType() == BattleElementType.FORTIFICATION) {
                ...
                if(battleFort.isDestroyed()) continue;
                if(battleFleet.isAttacker() &&
                   match.fortressAttackType() != AttackSideType.ATTACKER) continue;
                if(battleFleet.isDefender() &&
                   match.fortressAttackType() != AttackSideType.DEFENDER) continue;
    ...
    public BattleElement calculateMeantimeTarget(BattleFleet battleFleet, List<BattleElement> temporalElements, int minRange, int maxRange) {
        ...
        for(BattleElement element : temporalElements)
            if(element.getType() == BattleElementType.FORTIFICATION) {
                ...
                if(fort.getFortType().isStation()) continue;
    ...
    public boolean attackFleetByFleet(int steps, BattleFleet attacker, BattleFleet defender, FleetAttackFleetTrigger trigger) {
        ...
        for(int attackerSegmentedPos = 0; attackerSegmentedPos < 3; attackerSegmentedPos++) {
            attackerPosLoop : for (int attackerPosIndex = 0; attackerPosIndex <  3; attackerPosIndex++) {
                ...
                int finalAttackerSegmentedPos = attackerSegmentedPos;
                int finalAttackerPosIndex = attackerPosIndex;
        ...
        // interrupted = true;
        return trigger.getAttacks().stream().filter(attackMeta -> attackMeta.isAttack()).findAny().isPresent();
    ...
    public boolean attackFortByFleet(BattleFleet attacker, BattleFort defender, FleetAttackFortTrigger trigger) {
        ...
        attackerSegmentedLoop : for(int attackerSegmentedPos = 0; attackerSegmentedPos < 3; attackerSegmentedPos++) {
            attackerPosLoop: for (int attackerPosIndex = 0; attackerPosIndex < 3; attackerPosIndex++) {
    ...
    public boolean attackFleetByFort(BattleFort attacker, BattleFleet defender, FortAttackFleetTrigger trigger) {
        ...
        FortAttackCalculator calculator = new FortAttackCalculator(this, attacker, defender, trigger);
    ...
    public BattleCell[][] getCells(List<BattleElement> temporalElements) {
        ...
        for(BattleElement element : temporalElements) {
            ...
            if(element.getType() == BattleElementType.FLEET) cell.addFleet((BattleFleet) element);
            else {
    ...
    private void addBigFort(BattleCell[][] cells, BattleFort fort, int posX, int posY) {
        ...
        if(cells[posX][posY] == null) cells[posX][posY] = new BattleCell();
