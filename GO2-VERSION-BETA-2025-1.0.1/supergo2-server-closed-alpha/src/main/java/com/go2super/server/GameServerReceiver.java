...
public class GameServerReceiver extends SmartServer implements Runnable {
    ...
    public void tick() {
        ...
        /*if(getInputStream().available() < 4) {

            now = System.currentTimeMillis();
            diff = now - last;

            System.out.println("diff: " + diff);
            System.out.println("isClosed = " + getSocket().isClosed());
        ...
        if(!PacketRouter.getInstance().containsPacket(type)) {
            if(type > 0 && type < 5000) BotLogger.packet("⚠ NOT FOUND " + type);
            return;
        }
    ...
    public void close() {
        ...
        if(optionalSession.isPresent()) GameServer.clients.remove(optionalSession.get());
        ...
        if(optionalGameUser.isPresent()) {
            ...
            // ? Disconnect user!
            LoginService.getInstance().disconnectGame(optionalGameUser.get());
