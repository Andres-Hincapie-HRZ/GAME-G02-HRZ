...
public class PropsJson {
    ...
    public List<PropData> getInSell() {
        if(inSell.size() == 0)
            for(PropData data : props)
                if(data.hasMallData()) {
                    // Exclude testing packs by name or known IDs
                    String name = data.getName();
                    if(name != null && name.toLowerCase().contains("testing pack")) continue;
                    int id = data.getId();
                    // Expanded range to exclude all testing pack IDs (4600-4699)
                    if(id >= 4600 && id <= 4699) continue;
                    inSell.add(data);
