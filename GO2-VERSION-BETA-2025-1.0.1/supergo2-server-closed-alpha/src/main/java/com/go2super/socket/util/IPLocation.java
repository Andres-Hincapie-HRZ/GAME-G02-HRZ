...
public class IPLocation {
    ...
    public static Optional<IPLookupDTO> getLocation(String ip) {
        ...
        HttpResponse<JsonNode> response = Unirest.get(main_host)
                .asJson();

        String isp = null;
        String country = null;

        main : if(response.getStatus() == 200) {
        ...
        if(isp == null) return Optional.empty();
        else return Optional.ofNullable(IPLookupDTO.builder()
                .ip(ip)
                .country(country)
                .isp(isp)
                .build());
