...
public class PacketRouter {
    ...
    public String getPacketName(int packetId) {
        return packetsMap.get(packetId).getSimpleName();
    }
    ...
    public void playPacket(int size, int type, Go2Buffer buffer, Socket socket, SmartServer smartServer) {
        ...
        try {
            ...
            if(!Arrays.asList(502, 503, 1017).contains(type))
                BotLogger.packet("↙ FOUND " + type + (guid != null ? " (ID = " + guid + ")" : "") + " " + packetClass.getSimpleName() + " [P:" + JobService.getInstance().getThreadPool().getActiveCount() + "]");
