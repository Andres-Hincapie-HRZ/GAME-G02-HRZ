...
public class FortificationLevelMeta extends JsonData {
    ...
    public int getEffectValue(String name) {
        ...
        if(effect.isPresent()) return (int) effect.get().getValue();
        return -1;
    }
    ...
    public Optional<Integer> getWealthRequired() {
        ...
        if(wealthEffect.isPresent()) return Optional.of((int) wealthEffect.get().getValue());

        Optional<FortificationEffectMeta> needRichesEffect = getEffect("needRiches");
        if(needRichesEffect.isPresent()) return Optional.of((int) needRichesEffect.get().getValue());
