...
public class InstanceData extends JsonData {
    ...
    public synchronized List<BattleFleet> getEnemyFleets(InstanceMatch match) {
        ...
        if(match.getType() == InstanceType.INSTANCE) prefabNormalInstance(match.getId(), prefab);
        else if(match.getType() == InstanceType.RESTRICTED) prefabNormalInstance(match.getId(), prefab);
        else if(match.getType() == InstanceType.TRIALS) prefabTrialsInstance(match.getId(), prefab);
        else if(match.getType() == InstanceType.CONSTELLATION) prefabConstellationInstance(match.getId(), prefab);
    ...
    private synchronized void prefabWarInstance(int galaxyId, List<BattleFleet> prefab, boolean attacker, boolean techs) {
        ...
        for(int i = 0; i < 25; i++)
            for(int j = 0; j < 25; j++)
                ...
                else cells.add(GameCell.of(i, j));
