...
// Clase que representa una nave en construcción en la fábrica
public class FactoryShip {
    ...
    private double buildTime; // Tiempo total de construcción
    private double incSpeed; // Incremento de velocidad de construcción
    ...
    public FactoryShip(int shipModelId, int num, double buildTime) {
        ...
        this.until = DateUtil.now((int) buildTime); // Calcula fecha de finalización
    }
    ...
    public CreateShipInfo packet() {
        ...
        return new CreateShipInfo(shipModelId, remains + ((num - 1)  * buildTime), num, (int) incSpeed); // Crea info con parámetros calculados
    }

    // Método estático para crear instancia de FactoryShip
    public static FactoryShip of(int shipModelId, int num, double buildTime) {
        return new FactoryShip(shipModelId, num, buildTime); // Retorna nueva instancia
    }
