...
public class TransitionJob implements OfflineJob {
    ...
    public void attackPlanet(Planet planet, FleetTransmission transmission, Fleet fleet, User owner, GalaxyTile from, GalaxyTile to, List<Fleet> initiators) {
        ...
        if(transmission.getJumpType() == JumpType.RECALL || transmission.getGalaxyId() == owner.getGalaxyId()) {
            ...
            // * Initialize the war
            if(!planet.isInWar()) initiators.add(fleet);
        ...
        if(planet.getType() == PlanetType.USER_PLANET) spaceEntrance = to.getEntrance(from);
    ...
    public void run() {
        ...
        if(transitionFleets.isEmpty()) return;
        ...
        for(Fleet fleet : transitionFleets) {
            ...
            if(DateUtil.remains(transmission.getUntil()).intValue() <= 0) toUpdate.add(fleet.getShipTeamId());
        ...
        if(toUpdate.size() > 0) {
            ...
            JobService.submit(() -> {
                ...
                for(Fleet fleet : toUpdateFleets) {
                    ...
                    if(planet == null) invalid = true;
                    else if(Arrays.asList(PlanetType.HUMAROID_PLANET).contains(planet.getType())) {
                        HumaroidPlanet humaroidPlanet = (HumaroidPlanet) planet;
                        if(humaroidPlanet.hasTruce() || humaroidPlanet.isInWar()) invalid = true;
                    }
                        ...
                        if(optionalUser.isPresent() && optionalUser.get().getStats().hasTruce()) {
                            ...
                            if(targetUser.getGuid() != owner.getGuid()) {
                                if(targetUser.getConsortiaId() == -1) invalid = true;
                                else if(targetUser.getConsortiaId() != owner.getConsortiaId()) invalid = true;
                            }
                        }
                    } else if(planet.getType() == PlanetType.RESOURCES_PLANET) {
                        ResourcePlanet resourcePlanet = (ResourcePlanet) planet;
                        if(resourcePlanet.hasTruce()) invalid = true;
                    }
                    ...
                    if(invalid) {
                        ...
                        if(optionalLoggedGameUser.isPresent()) optionalLoggedGameUser.get().getSmartServer().send(response);
                        continue;
                ...
                for(Fleet initiator : initiators) {
                    ...
                    if(makeWars.stream().anyMatch(war -> war.getGalaxyId() == initiator.getGalaxyId())) continue;
