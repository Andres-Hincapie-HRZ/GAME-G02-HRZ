...
public class UpgradeJob implements GalaxyUserJob {
    ...
    public boolean needUpdate(User user) {
        ...
        if(upgrades.getShipUpgrade() == null && upgrades.getPartUpgrade() == null) return false;
    ...
    public boolean run(LoggedGameUser loggedGameUser, User user) {
        ...
        if(update) loggedGameUser.getSmartServer().send(ResourcesService.getInstance().getPlayerResourcePacket(user));
        return update;
