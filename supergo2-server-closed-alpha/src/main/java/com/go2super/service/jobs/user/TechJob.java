...
public class TechJob implements OfflineJob {
    ...
    public void run() {
        ...
        for(User user : users) {
            ...
            if(user.getTechs().getUpgrade() == null) continue;
            if(user.getTechs().getUpgrade().getUntil() == null) continue;

            if(DateUtil.remains(user.getTechs().getUpgrade().getUntil()).intValue() <= 0) toUpdate.add(user.getGuid());
