...
public class CorpUpgradeJob implements OfflineJob {
    ...
    public void run() {
        ...
        for(Corp corp : corps) {
            ...
            if(corp.getCorpUpgrade() == null) continue;
            if(DateUtil.remains(corp.getCorpUpgrade().getUntil()).intValue() <= 0) toUpdate.add(corp.getCorpId());
        ...
        if(toUpdate.size() > 0) {
            ...
            JobService.submit(() -> {
                ...
                for(Corp corp : toUpdateCorps) {
                    ...
                    if(corp == null || corp.getCorpUpgrade() == null) continue;
