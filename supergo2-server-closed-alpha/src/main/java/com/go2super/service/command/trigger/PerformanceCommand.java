...
public class PerformanceCommand extends Command {
    ...
    public void execute(User user, Account account, SmartServer smartServer, String label, String[] parts) {
        ...
        smartServer.sendMessage("Queue: " + JobService.getInstance().getThreadPool().getActiveCount());
        // smartServer.sendMessage("~ (" + JobService.getInstance().getHighestTask().getTaskName() + ", " + JobService.getInstance().getHighestTask().getTime() + " ms)");
        // smartServer.sendMessage("Min Peak >> ");
        // smartServer.sendMessage("~ (" + JobService.getInstance().getLowestTask().getTaskName() + ", " + JobService.getInstance().getLowestTask().getTime() + " ms)");
        smartServer.sendMessage("Battles = " + BattleService.getInstance().getBattles().size());
        // smartServer.sendMessage("Tasks List = " + JobService.getInstance().getTasks().size());
