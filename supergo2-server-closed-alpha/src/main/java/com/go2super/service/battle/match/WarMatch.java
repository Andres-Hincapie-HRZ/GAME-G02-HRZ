...
public class WarMatch extends Match {
    ...
    public void stop(StopCause cause) {
        ...
        defender : if(result == BattleResultType.LOSE && DefendJob.getInstance().getCurrentFlagGalaxyId() == getGalaxyId()) {
            ...
            for(BattleFleet battleFleet : getFleets()) {
                ...
                if(battleFleet.getGuid() == -1 || awarded.contains(battleFleet.getGuid())) continue;
                if(battleFleet.isDefender()) continue;

                User user = UserService.getInstance().getUserCache().findByGuid(battleFleet.getGuid());
                if(user == null) continue;

                if(user.getConsortiaId() == DefendJob.getInstance().getCurrentFlagCorp()) continue;
                ...
                if(corp != null && instanceData != null) {
                    ...
                    for(CorpMember member : corp.getMembers().getMembers()) {
                        ...
                        if(optionalUser.isPresent()) {
                            ...
                            if(onlineUser.isPresent()) onlineUser.get().getSmartServer().send(packet);
        ...
        if(fromPlanet.getType() == PlanetType.USER_PLANET) {
            ...
            if(optionalUser.isPresent()) {
                ...
                for(BattleShipCache shipCache : battleReport.getShipHistoric()) {
                    ...
                    if(shipCache.getShootdowns() <= 0) continue;
                    totalShootdowns += shipCache.getShootdowns();

                    if(shootdownsOverall.containsKey(shipCache.getGuid())) shootdownsOverall.put(shipCache.getGuid(), (int) (shootdownsOverall.get(shipCache.getGuid()) + shipCache.getShootdowns()));
                    else shootdownsOverall.put(shipCache.getGuid(), (int) shipCache.getShootdowns());
                ...
                for(Map.Entry<Integer, Integer> entry : shootdownsOverall.entrySet()) {
                    ...
                    if(result == BattleResultType.LOSE && attacker && index < 10) {
                        ...
                        email.setSubject("War Rewards");
                        email.setEmailContent("You won the war and claimed these rewards:");
                    ...
                    if(topOneAttacker && result == BattleResultType.LOSE) {
                        ...
                        if(topCorp != null && destroyedCorp != null) {
                            ...
                            for(BattleFleet cacheFleet : getFleets()) {
                                ...
                                if(cacheFleet.getGuid() == -1) continue;
                                if(cacheFleet.isDefender()) continue;
                                ...
                                if(cacheUser.getFlag() == null) continue;
                                if(cacheUser.getFlag().getCorpId() != destroyedCorp.getCorpId()) continue;
                                if(cacheUser.getCorp().getCorpId() != topCorp.getCorpId()) continue;
                ...
                while(iterator.hasNext()) {
                    ...
                    if(user != null) {
                        ...
                        if(optional.isPresent()) {
                            ...
                            PropData defResult = ResourceManager.getProps().getData(rewardMeta.getProp());
                            if(defResult != null) rewards.add(Pair.of(defResult, rewardMeta.getNum()));
    ...
    public void returnAllFleets() {
        ...
        if(currentPlanet.getType() == PlanetType.USER_PLANET) {
            ...
            Optional<User> optionalUser = userPlanet.getUser();
            if(optionalUser.isPresent()) currentGuid = optionalUser.get().getGuid();
        }

        for(BattleFleet userFleet : getAllFleets()) {

            if(userFleet.isPirate()) continue;
            ...
            if(commander != null) commander.save();

            if(fleet.getGuid() == currentGuid || userFleet.isDefender()) {
        ...
        for(int guid : userGuids) {
            ...
            List<Fleet> fleets = toReturn.stream().filter(fleet -> fleet.getGuid() == guid).collect(Collectors.toList());
    ...
    public BattleResultType getResultType() {
        ...
        if(attackers && defenders) return BattleResultType.WIN;
        if(!attackers && defenders) return BattleResultType.WIN;
        if(attackers) return BattleResultType.LOSE;
