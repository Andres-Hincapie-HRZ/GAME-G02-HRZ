...
public abstract class Match {
    ...
    public Set<LoggedGameUser> getViewers() {
        ...
        if(getMatchType().isVirtual() || getGalaxyId() == -1) viewers.addAll(LoginService.getInstance().getMatchViewers(id));
        else viewers.addAll(Stream.concat(LoginService.getInstance().getMatchViewers(id).stream(), LoginService.getInstance().getPlanetViewers(getGalaxyId()).stream())
                     .collect(Collectors.toList()));
        return viewers;
    }
