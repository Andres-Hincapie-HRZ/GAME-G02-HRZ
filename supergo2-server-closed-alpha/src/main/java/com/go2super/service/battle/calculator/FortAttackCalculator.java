...
public class FortAttackCalculator {
    ...
    private BattleFort attackerFort;
    private BattleFleet defenderFleet;

    private FortAttackFleetTrigger trigger;

    public FortAttackCalculator(MatchRunnable matchRunnable, BattleFort attacker, BattleFleet defender, FortAttackFleetTrigger trigger) {
    ...
    public FortAttackCalculator calculate(int uniqueDamage, List<ShipPosition> positions, FortAttackFleetTrigger trigger) {
        ...
        if(targetCells.isEmpty()) return this;

        int equallyDamage = (uniqueDamage / targetCells.size());
        ...
        for(ShipPosition position : positions) {
            ...
            calculate(equallyDamage, position, attackMeta, trigger);
    ...
    private FortAttackCalculator calculate(int equallyDamage, ShipPosition position, FortCellAttackMeta attackMeta, FortAttackFleetTrigger trigger) {

        String attackType = "fort";
        ...
        if(!defenderCell.hasShips()) return this;
        ...
        for(Map.Entry<Integer, List<BattleFleetDefensiveModule>> mitigateModule : mappedMitigations.entrySet()) {
            ...
            for(BattleFleetDefensiveModule defensiveModule : mitigateModule.getValue()) {
                ...
                double kindNegation = defensiveModule.getNegation(attackType);

                shipMitigation.add(defensiveModule, baseNegation, attackType, kindNegation);
        ...
        double finalTrueReflectionDamage = trueReflectionDamage;
        double finalDefenderEffectiveStack = defenderEffectiveStack;
        ...
        double agility = defenderStats.getAgility();
        double hitRate = 1.0d;
        ...
        // ? Calculate damages
        double calculatedDamage = equallyDamage;
