...
public class LoginService {
    ...
    private CopyOnWriteArrayList<LoggedGameUser> gameUsers = new CopyOnWriteArrayList<>();
    private CopyOnWriteArrayList<LoggedSessionUser> sessionUsers = new CopyOnWriteArrayList<>();
    // private List<LoggedSessionAccount> sessionAccounts = new ArrayList<>();

    private SessionService sessionService;
    private EmailService emailService;

    @Getter AccountCache accountCache;
    @Getter UserCache userCache;

    @Autowired
    public LoginService(UserCache userCache, AccountCache accountCache, SessionService sessionService, EmailService emailService) {
    ...
    public CopyOnWriteArrayList<LoggedGameUser> getGameUsers() {
        return gameUsers;
    }
    ...
    public Optional<LoggedSessionAccount> getSessionAccount(String token) {
        ...
        if(!accountSessionOptional.isPresent())
    ...
    public void disconnectGame(LoggedSessionUser user) {
        ...
        for(LoggedGameUser loggedGameUser : LoginService.getInstance().getGameUsers())
            if(loggedGameUser.getUserId() == user.getUser().getPlanet().getUserId()) {
                ...
                buffer.append("**Logout:** `" + user.getUser().getUsername() + " (ID: " + user.getUser().getGuid() + ", EMAIL: " + account.getEmail() + ")`\n");

                DiscordService.getInstance().getRayoBot().sendAudit(buffer.toString(), "", Color.red, AuditType.LOGIN);
                break;
    ...
    public BasicResponse login(AccountLoginDTO dto) {
        ...
        if(Crypto.decrypt(account.getPassword()).equals(dto.getPassword())) {
            ...
            /*Optional<AccountSession> oldLoggedAccount = sessionService.getActiveAccountSessionByAccountId(account.getId().toString());

            if(oldLoggedAccount.isPresent()){

                List<User> users = userCache.findByAccountId(oldLoggedAccount.get().getAccountId());
                ...
                users.stream().forEach(user -> {
                    ...
                    Optional<LoggedSessionUser> loggedSessionUser =
                            sessionUsers
                            .stream()
                            .filter(session -> session.getUser().getUserId() == user.getUserId())
                            .findAny();
                ...
                disconnectWeb(oldLoggedAccount.get().getToken());
...
//        } else {
//
//            String otp = emailService.sendOTP(dto.getEmail());
//            if(otp == null)
//                return BasicResponse
//                        .builder()
//                        .code(HttpStatus.NOT_ACCEPTABLE.value())
//                        .message("INTERNAL_ERROR_DURING_EMAIL_SEND")
//                        .build();
//
//            registerValidations.put(dto.getEmail(), RegisterValidation.builder()
//                    .otp(otp)
//                    .until(DateUtil.now(60 * 15))
//                    .build());
//
//            return BasicResponse
//                    .builder()
...
//        sb.append("&secret=");
//        sb.append(PacketService.getInstance().getHcaptchaSecret());
//
...
//
//        HttpClient build = HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(5))
//                .build();
//
//        HttpResponse<String> response = build.send(request,
...
//                    .build();
//
//        JSONObject hcaptchaResponse = new JSONObject(response.body());
//
//        if(!hcaptchaResponse.has("success"))
//            return BasicResponse
//                    .builder()
//                    .code(HttpStatus.NOT_ACCEPTABLE.value())
//                    .message("HCAPTCHA_ERROR_2")
//                    .build();
//
//        boolean success = hcaptchaResponse.getBoolean("success");
//
//        if(!success)
//            return BasicResponse
//                    .builder()
...
//                    .build();
        ...
        // AccountService.getInstance().createTestUser(newAccount);
    ...
    public static void validate(Packet packet, int guid) throws BadGuidException {
        ...
        if(!(packet.getSmartServer() instanceof GameServerReceiver))
        ...
        if(serverReceiver.getGuid() != guid) invalid = true;
