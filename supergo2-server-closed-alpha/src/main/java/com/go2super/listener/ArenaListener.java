    ...
    public void onArenaStatus(RequestArenaStatusPacket packet) throws BadGuidException {
        ...
        if(packet.getRoomId() == -1) {
            ...
            if(optionalMatch.isEmpty()) {
                ...
                if(match.getMatchType() != MatchType.ARENA_MATCH) return;
        ...
        } else optionalArenaMatch = BattleService.getInstance().getArenaByRoomId(packet.getRoomId());

        if(!optionalArenaMatch.isPresent()) return;
        ...
        if(request == 0) {
            ...
            if(packet.getGuid() != arenaMatch.getSourceGuid() && packet.getGuid() != arenaMatch.getTargetGuid()) return;

            if(!arenaMatch.isPause()) return;
            if(arenaMatch.getSourceGuid() != user.getGuid()) return;
