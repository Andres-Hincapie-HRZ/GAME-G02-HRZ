...
public class OnlineListener implements PacketListener {
    ...
    public void onOnlineAward(RequestGetOnlineAwardPacket packet) throws BadGuidException {
        ...
        if(userRewards == null || userRewards.getLevel() == -1) return;

        RewardsJson rewardsJson = ResourceManager.getRewards();
        boolean reset = false;

        if(user.getLastDayUpdate() != null && !DateUtil.currentDay(user.getLastDayUpdate())) reset = true;
        if(userRewards.getUntil() == null) reset = true;
    ...
    private List<Packet> giveReward(RewardData rewardData, User user) {
        ...
        switch(rewardData.getReward().getType()) {
            ...
            case "propPoolUnlock":
                ...
                Packet unlockedPoolProp = ResourcesService.getInstance().smartUseProps(-1, 0, 0, 0, 0, 0, 1, 0, Arrays.asList(Pair.of(propPoolUnlockId, propPoolUnlockNum)));
        ...
        if(propsId != -1) responses.add(preparePacket(propsId, num));
        return responses;
