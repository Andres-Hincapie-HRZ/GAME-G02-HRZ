...
public class GalaxyListener implements PacketListener {
    ...
    private Map<Integer, Integer> lastSequence = new HashMap<>();
    ...
    public void onGalaxy(RequestGalaxyPacket packet) throws BadGuidException {
        ...
        if(planet.getType() == PlanetType.RESOURCES_PLANET) {
            ...
            if(resourcePlanet.isInWar()) {
                ...
                if(userCorp == null || rbpCorp.isEmpty()) return;
                if(userCorp.getCorpId() != rbpCorp.get().getCorpId()) return;
    ...
    public void joinGalaxyBroadcast(LoggedGameUser gameUser, User user) {
        ...
        if(gameUser.getViewing() != -1) receivers.addAll(LoginService.getInstance().getPlanetViewers(gameUser.getViewing()));
        else if(gameUser.getMatchViewing() != null) receivers.addAll(LoginService.getInstance().getMatchViewers(gameUser.getMatchViewing()));
