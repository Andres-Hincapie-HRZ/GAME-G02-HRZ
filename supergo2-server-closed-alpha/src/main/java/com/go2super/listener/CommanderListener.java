...
public class CommanderListener implements PacketListener {
    ...
    public void onEditFleet(RequestCommanderEditShipTeamPacket packet) {
        ...
        if(fleet == null || fleet.getGuid() != packet.getGuid()) return;
        if(fleet.isMatch() || fleet.isInMatch()) return;
        ...
        int total = 0;
        if(newTeamBody.getCells().size() != 9) return;
        ...
        response.setKind((byte) packet.getEditType());
    ...
    public void onCommanderInsertStone(RequestCommanderInsertStonePacket packet) throws BadGuidException {
        ...
        if(packet.getGemType() != 0 && packet.getGemType() != 1) return;
        if(!(packet.getHoleId() >= 0 && packet.getHoleId() <= 11)) return;

        Commander commander = user.getCommander(packet.getCommanderId());
        if(commander == null) return;

        int maxGems = commander.getLevel().getLevelData().getGem();
        if(maxGems < packet.getHoleId() + 1) return;
        ...
        if(packet.getGemType() == 0) {
            ...
            if(!data.getType().equals("gem")) return;

            PropGemData gemData = data.getGemData();
            if(gemData == null) return;

            int validation = validator[packet.getHoleId()];
            if(validation != -1 && gemData.getColor() != validation) return;
            if(gems.get(packet.getHoleId()) != -1) return;
            if(!user.getInventory().removeProp(data.getId(), 1, 0, packet.getLockFlag() == 1)) return;
    ...
    public void onUnionCommanderCard(RequestUnionCommanderCardPacket packet) throws BadGuidException {
        ...
        if(packet.getCard3() == -1 && packet.getCard1() != packet.getCard2()) return;
        ...
        if(mergeData == null || nextData == null || mergeData != nextData) return;
        ...
        int success = 100 + chipSuccess + corpMergeBonus;
        if(resultStars >= 3) success = Math.max(success - (resultStars - 2) * 10, 0);

        boolean failed = false;
        if(MathUtil.random(0, 100) > success) failed = true;

        if(packet.getCard3() == -1) {

            Prop prop = user.getInventory().getProp(mergeCard);

            if(prop == null || prop.getPropNum() < 2) return;
            if(!user.getInventory().removeProp(prop, failed ? 1 : 2, false)) return;
            if(!failed && !user.getInventory().addProp(nextCard, 1, 0, false)) return;
            if(!failed) user.getMetrics().add("action:combine.cards", 1);
        ...
        if(card2 == null || card3 == null) return;

        int stars3 = packet.getCard3() - card3.getId();
        if(stars1 != stars2 || stars1 != stars3) return;

        if(!card2.getCommanderData().getCommander().getType().equals(mergeData.getCommanderData().getCommander().getType()) ||
           !card3.getCommanderData().getCommander().getType().equals(mergeData.getCommanderData().getCommander().getType())) return;

        if(!failed && !user.getInventory().removeProp(packet.getCard1(), 1, 0, false)) return;
        if(!user.getInventory().removeProp(packet.getCard2(), 1, 0, false)) return;
        if(!user.getInventory().removeProp(packet.getCard3(), 1, 0, false)) return;
        if(!failed && !user.getInventory().addProp(nextCard, 1, 0, false)) return;
        if(!failed) user.getMetrics().add("action:combine.cards", 1);
    ...
    public void onCommanderUnionStone(RequestCommanderUnionStonePacket packet) throws BadGuidException {
        ...
        PropGemData gemData = data.getGemData();
        if(gemData.getLevel() >= 4) return;

        boolean lock = packet.getLockFlag() != 0;
        if(!user.getInventory().removeProp(gem, 4, lock)) return;
        ...
        if(!user.getInventory().addProp(result, 1, 0, lock)) return;
        ...
        if(packet.getSmartServer().getSocket().isConnected()) packet.reply(response);

        if(bro) PacketRouter.getInstance().broadcast(response, user);
        return;
    ...
    public void onCommanderPropertyStone(RequestCommanderPropertyStonePacket packet) throws BadGuidException {
        ...
        if(objGemData == null || srcData1 == null || srcData2 == null) return;
        if(!objGemData.getType().equals("gem") || !srcData1.getType().equals("gem") || !srcData2.getType().equals("gem")) return;
        if(srcData3 != null && !srcData3.getType().equals("gem")) return;
        ...
        if(!user.getInventory().removeProp(srcData1.getId(), 1, 0, locked)) return;
        if(!user.getInventory().removeProp(srcData2.getId(), 1, 0, locked)) return;
        if(srcData3 != null && !user.getInventory().removeProp(srcData3.getId(), 1, 0, locked)) return;

        if(!user.getInventory().addProp(objGemData.getId(), 1, 0, locked)) return;
        if(user.getResources().getGold() < 10000) return;
    ...
    public void onCommanderSeal(RequestCommanderChangeCardPacket packet) throws BadGuidException {
        ...
        Commander commander = user.getCommander(packet.getCommanderId());
        if(commander == null || commander.getChips().size() > 0 || commander.hasGems() || commander.isCommon()) return;
        ...
        buffer.append("**User:** `" + user.getUsername() + " (ID: " + user.getGuid() + ", EMAIL: " + account.getEmail() + ")`\n");
        buffer.append("**Commander:** `" + commander.getName() + " (CommanderId: " + commander.getCommanderId() + ", Stars: " + (commander.getStars() + 1) + ", Electron: " + commander.getGrowthElectron() + ", Dodge: " + commander.getGrowthDodge() + ", Speed: " + commander.getGrowthSpeed() + ", Accuracy: " + commander.getGrowthAim() + ", Variance: " + commander.getVariance() + ")`");

        DiscordService.getInstance().getRayoBot().sendAudit("Commander Seal", buffer.toString(), Color.decode("0x1c27ff"), AuditType.COMMANDER);
    ...
    public void onResume(RequestResumeCommanderPacket packet) throws BadGuidException {
        ...
        Commander commander = user.getCommander(packet.getCommanderId());
        if(commander == null || commander.getState() != 1) return;
    ...
    public void onRelive(RequestReliveCommanderPacket packet) throws BadGuidException {
        ...
        Commander commander = user.getCommander(packet.getCommanderId());
        if(commander == null || commander.getState() != 2) return;
    ...
    public void onReset(RequestClearCommanderPercentPacket packet) throws BadGuidException {
        ...
        Commander commander = user.getCommander(packet.getCommanderId());
        if(commander == null || commander.hasGems()) return;

        Fleet fleet = commander.getFleet();
        if(fleet != null && (fleet.isInMatch() || fleet.isMatch() || fleet.isInTransmission())) return;
        ...
        DiscordService.getInstance().getRayoBot().sendAudit("Commander Reset", buffer.toString(), Color.decode("0x2bdfff"), AuditType.COMMANDER);
    ...
    public void onDeleteCommander(RequestDeleteCommanderPacket packet) throws BadGuidException {
        ...
        Commander commander = CommanderService.getInstance().getCommander(packet.getCommanderId());
        if(commander == null || commander.getUserId() != user.getUserId() || commander.hasGems() || commander.hasChips() || commander.getState() == 4 || commander.hasFleet()) return;
        ...
        DiscordService.getInstance().getRayoBot().sendAudit("Commander Dismiss", buffer.toString(), Color.decode("0xf54284"), AuditType.COMMANDER);
    ...
    private Optional<PropData> findUnionDouble(PropScrollData scrollData, PropCommanderData card1, PropCommanderData card2) {
        ...
        // System.out.println("ScrollData: " + scrollData);
        // System.out.println("Card1: " + card1);
        // System.out.println("Card2: " + card2);

        String commanderNameId1 = card1.getCommander().getName();
        String commanderNameId2 = card2.getCommander().getName();
        if(commanderNameId1.equals(commanderNameId2)) return Optional.empty();

        String[] skills = scrollData.getSrcCommander();
        if(skills == null || skills.length != 2) return Optional.empty();

        if(skills[0].equals(commanderNameId1) && skills[1].equals(commanderNameId2)) return scrollData.getPropDataResult();
        if(skills[0].equals(commanderNameId2) && skills[1].equals(commanderNameId1)) return scrollData.getPropDataResult();
