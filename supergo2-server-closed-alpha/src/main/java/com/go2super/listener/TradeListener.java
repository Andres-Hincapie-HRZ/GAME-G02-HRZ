...
public class TradeListener implements PacketListener {
    ...
    public void onDeleteTrade(RequestDeleteTradeGoodsPacket packet) throws BadGuidException {
        ...
        if(selectedTrade == null || selectedTrade.getSellerGuid() != user.getGuid()) return;
    ...
    public void onBuyTrade(RequestBuyTradeGoodsPacket packet) throws BadGuidException {
        ...
        Trade selectedTrade = TradeService.getInstance().getTradeCache().findByTradeId(packet.getIndexId());
        if(selectedTrade == null || selectedTrade.getSellerGuid() != packet.getSellGuid()) return;

        User userSeller = selectedTrade.getSeller();
        if(userSeller == null) return;
    ...
    public void onTradeGoods(RequestTradeGoodsPacket packet) throws BadGuidException {
        ...
        if(tradeBuilding == null) return;

        if(packet.getPrice() <= 0 || packet.getNum() <= 0) return;
        ...
        if(fee <= 0) fee = 1;
        boolean isMp = packet.getPriceType() == 1 ? true : false;
        ...
        switch (tradeType) {
            ...
            case SHIP -> {
                ...
                if(shipTeamNum == null || shipTeamNum.getNum() == 0) return;
                if(shipTeamNum.getNum() < packet.getNum()) return;

                shipTeamNum.setNum(shipTeamNum.getNum() - packet.getNum());
                if(shipTeamNum.getNum() <= 0) userShips.getShips().remove(shipTeamNum);
            ...
            case ITEM -> {
                ...
                if(prop == null) return;
                if(prop.getPropNum() < packet.getNum()) return;
