...
public class ChatListener implements PacketListener {
    ...
    public static final Map<Integer, String> blockSpy = new HashMap<>();
    ...
    public void onChat(ChatMessagePacket packet) throws BadGuidException {
        ...
        String value = packet.getBuffer().getValue().replaceAll("[^\\p{L}|:|_|.|/|&|\\*|%|=|)|(|\\[|\\]|~|^\\p{N}|$|#|\"|\'|\\,|\\-|+|\s|&|?|¿|!|¡|\\^|;]", "");
        packet.getBuffer().value(value);
        ...
        switch (packet.getChannelType()) {
            case CHANNEL_GLOBAL:
                ...
                // Audit
                DiscordService.getInstance().getRayoBot().sendAudit("[World] " + user.getUsername() + " ~ " + user.getGuid(), value, Color.decode("0xfafafa"), AuditType.CHAT);
            ...
            case CHANNEL_PLANET:
                ...
                // Audit
                DiscordService.getInstance().getRayoBot().sendAudit("[Planet] " + user.getUsername() + " ~ " + user.getGuid(), value, Color.decode("0xf2c479"), AuditType.CHAT);
            ...
            case CHANNEL_CORPS:
                ...
                if(corpFetch != null) corpName = corpFetch.getName();

                DiscordService.getInstance().getRayoBot().sendAudit("[Corp / " + corpName + "] " + user.getUsername() + " ~ " + user.getGuid(), value, Color.decode("0xb4ff69"), AuditType.CHAT);

                break;

            case CHANNEL_PRIVATE:

                if (userToSend.isNotDisturb() == true) {
                ...
                // Audit
                DiscordService.getInstance().getRayoBot().sendAudit("[Private] " + user.getUsername() + " ~ " + user.getGuid() + " > " + userToSend.getUsername() + " ~ " + userToSend.getGuid(), value, Color.decode("0xfd69ff"), AuditType.CHAT);
            ...
            case CHANNEL_TEAM:
                ...
                } else if(userCorp.getBlocId() != null && !userCorp.getBlocId().isEmpty()) {
                ...
                if(corpMember.getRank() == 1) roleName = userCorp.getConsortiaJobName().getName1();
                else if(corpMember.getRank() == 2) roleName = userCorp.getConsortiaJobName().getName2();

                if(userCorp != null && userCorp.getAcronym() != null && userCorp.getAcronym().length() == 3)
                    roleName = "[" + userCorp.getAcronym() + "]" + " (" + SmartString.noSpaces(roleName) + ")";
                else roleName = "(" + SmartString.noSpaces(roleName) + ")";
                ...
                for(Map.Entry<Integer, String> spy : blockSpy.entrySet()) {

                    if(!spy.getValue().equals(blockId)) continue;
                ...
                // Audit
                DiscordService.getInstance().getRayoBot().sendAudit("[Bloc] " + user.getUsername() + " ~ " + user.getGuid(), value, Color.decode("0x5adbf2"), AuditType.CHAT);
