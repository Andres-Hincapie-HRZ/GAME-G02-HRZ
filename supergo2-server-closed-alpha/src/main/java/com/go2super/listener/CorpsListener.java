...
public class CorpsListener implements PacketListener {
    ...
    public void onConsortiaInfo(RequestConsortiaInfoPacket packet) throws BadGuidException {
        ...
        if(building.isEmpty() || building == null) return;
    ...
    public void onCreateConsortia(RequestCreateConsortiaPacket packet) throws BadGuidException {
        ...
        if(resources.getGold() < 10000) return;
        ...
        if(corpMemberVerify != null) return;
        ...
        if (searchCorp != null) {
            ...
            ResponseCreateConsortiaPacket response = ResponseCreateConsortiaPacket.builder()
                    ...
                    .errorCode((byte) 1)
                    .propsCorpsPack(-1)
                    .lockFlag((byte) (propRemove.getValue() == true ? 1 : 0))
                    .build();
        ...
        if(corpMember.getRank() == 1 || corpMember.getRank() == 2 || corpMember.getRank() == 3) {
            ...
            if (packet.getAgree() == 0) {
                ...
                if (gameUserOptional.isPresent())
                    gameUserOptional.get().getSmartServer().send(joinConsortiaPacket);
    ...
    public void onLeaveConsortia(RequestConsortiaDelMemberPacket packet) throws BadGuidException {
        ...
        if(delCorpMember == null) return;

        boolean leave = user.getGuid() == packet.getDelGuid();

        if(delCorpMember.getRank() == 1) return;
        if(!leave && corpMember.getRank() != 1 && corpMember.getRank() != 2) return;
    ...
    public void onConsortiaThrowRank(RequestConsortiaThrowRankPacket packet) throws BadGuidException {
        ...
        corpMembers.sort((member1, member2) -> {
            if(member1.getRank() > member2.getRank()) return -1;
            if(member1.getRank() < member2.getRank()) return 1;
            return 0;
        });
        ...
        for (int i = packet.getPageId() * 10; i < ((packet.getPageId() + 1) * 10); i++)
            if (corpMembers.size() > i) {
                ...
                if(userCorpMember == null) continue;
    ...
    public void onConsortiaThrowValue(RequestConsortiaThrowValuePacket packet) throws BadGuidException {
        ...
        if (packet.getKind() == 0) { // 1 -> Mall Points
            ...
            if(user.getResources().getMallPoints() < packet.getValue()) return;
        ...
        if (packet.getKind() == 1) { // 2 -> GOLD
            ...
            if(user.getResources().getGold() < value) return;
        ...
        if (packet.getKind() == 2) { // 2 -> METAL
            ...
            int value = packet.getValue() * 10000;
            if(user.getResources().getMetal() < value) return;
        ...
        if (packet.getKind() == 3) { // 3 -> He3
            ...
            int value = packet.getValue() * 10000;
            if(user.getResources().getHe3() < value) return;
    ...
    public void onConsortiaMySelf(RequestConsortiaMySelfPacket packet) throws BadGuidException {
        ...
        for (int i = 0; i < corpMembers.size(); i++) {
            ...
            if(userCorp == null) continue;

            if(userCorp.isOnline()) memberStatus = 1;
            else memberStatus = 0;
            ...
            if(prepared == null || (prepared.getData().size() + 1) > 18) {
                if(prepared != null) packet.reply(prepared);
    ...
    public void onConsortiaField(RequestConsortiaFieldPacket packet) throws BadGuidException {
        ...
        if(resourcePlanets.isEmpty() || resourcePlanets == null) return;
    ...
    public void onConsortiaUpdateJobName(RequestConsortiaUpdateJobNamePacket packet) throws BadGuidException {
        ...
        if(userCorpMember.getRank() != 1) return;
    ...
    public void onConsortiaGiven(RequestConsortiaGivenPacket packet) throws BadGuidException {
        ...
        if(leader == null || leader.getRank() != 1) return;
    ...
    public void onConsortiaSetOfficial(RequestConsortiaSetOfficialPacket packet) throws BadGuidException {
        ...
        if(corpMemberToTransfer == null) return;
    ...
    public void onConsortiaBuyGoods(RequestConsortiaBuyGoodsPacket packet) throws BadGuidException {
        ...
        CorpMember corpMember = corp.getMembers().getMember(user.getGuid());
        if(corpMember == null) return;
        if(corpMember.getContribution() < corp.getContributionMall()) return;
        ...
        if(packet.getNum() <= 0) return;
        if(corpMember.getContribution() < (data.getCost() * packet.getNum())) return;
    ...
    public void onConsortiaUpgrade(RequestConsortiaUpgradePacket packet) throws BadGuidException {
        ...
        if(corpMember == null) return;
        if(corpMember.getRank() != 1) return;
        if(corp.getCorpUpgrade() != null) return;

        if(packet.getKind() == 0) {

            if(corp.getLevel() >= 9) return;
            ...
            if(corpsLevelData == null) return;
            if(corpsLevelMeta == null) return;

            if(corp.getWealth() < corpsLevelMeta.getCost()) return;
            int time = PacketService.getInstance().isFastCorpUpgrade() ? 61 : corpsLevelMeta.getTime();
        ...
        if (packet.getKind() == 1) {
            ...
            if(corp.getWarehouseLevel() >= 9) return;
            ...
            if(corp.getWealth() < corpsLevelMeta.getCost()) return;
            if(corp.getLevel() <= corp.getWarehouseLevel()) return;
        ...
        if (packet.getKind() == 2) {
            ...
            if(corp.getMergingLevel() >= 9) return;
            ...
            CorpsLevelMeta corpsLevelMeta = corpsLevelData.getLevels().get(nextLevel);
            if(corp.getWealth() < corpsLevelMeta.getCost()) return;
            if(corp.getLevel() <= corp.getMergingLevel()) return;
        ...
        if (packet.getKind() == 3) {
            ...
            CorpsLevelMeta corpsLevelMeta = corpsLevelData.getLevels().get(nextLevel);
            if(corp.getWealth() < corpsLevelMeta.getCost()) return;
            if(corp.getLevel() <= corp.getMallLevel()) return;
    ...
    public void onConsortiaRank(RequestConsortiaRankPacket packet) throws BadGuidException {
        ...
        if(corpCount <= 0) return;

        List<Corp> corps = CorpService.getInstance().getCorpCache().findAll();
        corps.sort((corp1, corp2) -> {
            if(corp1.getResourcePlanets().size() > corp2.getResourcePlanets().size()) return -1;
            if(corp1.getResourcePlanets().size() < corp2.getResourcePlanets().size()) return 1;
            if(corp1.getWealth() > corp2.getWealth()) return -1;
            if(corp1.getWealth() < corp2.getWealth()) return 1;
            return 0;
        });
    ...
    public void onConsortiaPirateChoose(RequestConsortiaPirateChoosePacket packet) throws BadGuidException {
        ...
        if(userCorp == null) return;

        CorpMember userMember = userCorp.getMembers().getMember(user.getGuid());
        if(userMember == null || userMember.getRank() != 1) return;
        ...
        if(objCorp == null || objCorp.getCorpId() != userCorp.getCorpId()) return;
    ...
    public void onConsortiaPirate(RequestConsortiaPiratePacket packet) throws BadGuidException {
        ...
        User user = UserService.getInstance().getUserCache().findByGuid(packet.getGuid());
        if(user == null) return;
        if(packet.getLevel() < 0 || packet.getLevel() > 9) return;
        ...
        CorpMember objMember = objCorp.getMembers().getMember(objUser.getGuid());
        if(objMember == null) return;
        if(objUser.getPlanet().isInWar()) return;
        if(objUser.getStats().hasTruce()) return;
        ...
        if(!(objCorp.getLevel() > 1)) return;

        if(instanceData.getId() > 2 && (objCorp.getPiratesLevel() + 1) < instanceData.getId()) return;
    ...
    public void onGotoResourceStar(RequestGotoResourceStarPacket packet) throws BadGuidException {
        ...
        if(planet == null || planet.getType() != PlanetType.RESOURCES_PLANET) return;

        ResourcePlanet resourcePlanet = (ResourcePlanet) planet;
        if(resourcePlanet == null || !resourcePlanet.isPeace()) return;

        Optional<Corp> planetCorp = resourcePlanet.getCorp();
        if(planetCorp.isEmpty()) return;

        Corp corp = planetCorp.get();
        if(corp.getCorpId() != userCorp.getCorpId()) return;

        int maxFleets = resourcePlanet.getMaxFleets();
        if(maxFleets <= 0) return;

        List<Fleet> current = PacketService.getInstance().getFleetCache().findAllByGalaxyId(planet.getPosition().galaxyId());
        if(current.size() >= maxFleets || current.size() + packet.getDataLen() >= maxFleets) return;
    ...
    public void onConsortiaAttackInfo(RequestConsortiaAttackInfoPacket packet) throws BadGuidException {
        ...
        User user = UserService.getInstance().getUserCache().findByGuid(packet.getGuid());
        if(user == null || packet.getPageId() < 0) return;
        ...
        for(User member : members) domain.add(member.getPlanet());
        for(ResourcePlanet resourcePlanet : userCorp.getResourcePlanets()) domain.add(resourcePlanet);

        List<Fleet> transition = PacketService.getInstance().getFleetCache().getInTransmissionFleets();
        if(transition.isEmpty()) return;

        transition = transition.stream().filter(fleet -> fleet.getFleetTransmission() != null && domain.stream().anyMatch(planet -> planet.getPosition().galaxyId() == fleet.getFleetTransmission().getGalaxyId())).collect(Collectors.toList());
        transition = transition.stream().filter(fleet -> !members.stream().anyMatch(member -> member.getGuid() == fleet.getGuid())).collect(Collectors.toList());
        if(transition.isEmpty()) return;

        List<List<Fleet>> partition = Lists.partition(transition, 6);
        if(packet.getPageId() < partition.size());

        List<Fleet> page = partition.get(packet.getPageId());
        ...
        response.setAttackCount((short) page.size());
        ...
        for(Fleet fleet : page) {
            ...
            if(fleetTransmission == null) continue;
            ...
            if(ownerCorp != null) info.setConsortiaName(ownerCorp.getName());
            else info.setConsortiaName("");

            if(targetPlanet instanceof ResourcePlanet) {

                ResourcePlanet resourcePlanet = (ResourcePlanet) targetPlanet;
            ...
            } else if(targetPlanet instanceof UserPlanet) {

                UserPlanet userPlanet = (UserPlanet) targetPlanet;
