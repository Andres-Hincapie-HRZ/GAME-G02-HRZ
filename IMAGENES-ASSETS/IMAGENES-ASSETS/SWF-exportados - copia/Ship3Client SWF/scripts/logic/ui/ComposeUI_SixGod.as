package logic.ui
{
   ...
   import logic.reader.SixGodRead;
   import logic.utils.UpdateResource;
   import net.base.NetManager;
   import net.msg.FlagShip.MSG_REQUEST_UNIONSHIPPROPS;
   import net.msg.FlagShip.MSG_RESP_UNIONSHIPPROPS;
   
   public class ComposeUI_SixGod
   {
      
      private static var instance:ComposeUI_SixGod;
      ...
      public function ComposeUI_SixGod()
      {
         super();
      }
      
      public static function GetInstance() : ComposeUI_SixGod
      {
         if(!instance)
         {
            instance = new ComposeUI_SixGod();
         }
         return instance;
      }
      ...
      {
         this.BaseMc = param1;
         this.FlagShipMc = param1.getChildByName("mc_energe") as MovieClip;
      ...
      private function SelectedShipOver(param1:MouseEvent) : void
      {
         ...
         this.ShowPartTip(MovieClip(this.FlagShipMc.mc_mergelist6),this.SelectedFlagShip.PropsId);
      }
      ...
      private function InitFlagShip() : void
      {
         ...
         this.FlashShipList = SixGodRead.getInstance().m_sixGodAry;
         this.PageCount = Math.ceil(this.FlashShipList.length / 5);
         var _loc2_:Sprite = Sprite(this.FlagShipMc.mc_mergelist6);
      ...
      private function ShowFlagShip() : void
      {
         ...
         {
            _loc4_ = this.SelectedFlagShip.NeedParts[_loc2_];
            _loc5_ = this.ShipPartMcList[_loc2_];
            _loc6_ = Sprite(_loc5_.m_movie.mc_base);
            ...
            if(_loc4_ != null)
            {
               _loc5_.m_movie.gotoAndStop(2);
               _loc6_.addChildAt(new Bitmap(GameKernel.getTextureInstance(_loc4_.PropsInfo.ImageFileName)),0);
               _loc7_ = this.GerPartCount(_loc4_.PropsInfo.Id);
               TextField(_loc5_.m_movie.txt_num).text = _loc7_ + "/" + _loc4_.Num;
               TextField(_loc5_.m_movie.txt_num).textColor = _loc7_ >= _loc4_.Num ? 65280 : 16711680;
               _loc1_ &&= _loc7_ >= _loc4_.Num;
            }
            else
            {
               TextField(_loc5_.m_movie.txt_num).text = "";
         ...
         this.CanReset = false;
         var _loc3_:Sprite = Sprite(this.FlagShipMc.mc_mergelist6);
         if(_loc3_.numChildren > 0)
         {
            _loc3_.removeChildAt(0);
         }
         if(this.SelectedFlagShip.PropsId == -1)
         {
            _loc3_.addChildAt(new Bitmap(GameKernel.getTextureInstance("random")),0);
         }
         else
         {
            _loc3_.addChildAt(new Bitmap(GameKernel.getTextureInstance(this.SelectedFlagShip.PropsInfo.ImageFileName)),0);
         }
      ...
      private function GerPartCount(param1:int) : int
      {
         ...
         var _loc2_:int = 0;
         for each(_loc3_ in this.ShipPartArray)
         {
            if(_loc3_._PropsInfo.Id == param1)
            {
               _loc2_ += _loc3_.Num;
            }
         }
      ...
      private function mc_mergeClick(param1:MouseEvent) : void
      {
         ...
         var _loc3_:MSG_REQUEST_UNIONSHIPPROPS = new MSG_REQUEST_UNIONSHIPPROPS();
      ...
      public function Resp_MSG_RESP_UNIONSHIPPROPS(param1:MSG_RESP_UNIONSHIPPROPS) : void
      {
         ...
         for each(_loc3_ in this.FlashShipList)
         {
            if(_loc3_.PropsId == _loc2_)
            {
               for each(_loc4_ in _loc3_.NeedParts)
               {
                  if(_loc4_ != null)
                  {
                     UpdateResource.getInstance().DeleteProps_1(_loc4_.PropsId,1,_loc4_.Num);
                  }
               }
               break;
            }
         ...
         this.Clear(false);
         this.ShowFlagShip();
         ResPlaneUI.getInstance().PlayReward(param1.PropsId,Sprite(this.FlagShipMc.mc_mergelist6));
