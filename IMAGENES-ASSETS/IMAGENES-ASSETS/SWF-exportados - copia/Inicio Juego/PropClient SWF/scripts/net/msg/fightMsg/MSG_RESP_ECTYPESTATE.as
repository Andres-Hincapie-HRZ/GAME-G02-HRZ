package net.msg.fightMsg
{
   import flash.utils.ByteArray;
   import net.base.MsgSocket;
   import net.common.MsgHead;
   import net.common.MsgTypes;
   
   public class MSG_RESP_ECTYPESTATE extends MsgHead
   {
      
      public var EctypeId:int;
      
      public var GateId:int;
      
      public var state:int;
      
      public function MSG_RESP_ECTYPESTATE()
      {
         super();
         usSize = this.getLength();
         usType = MsgTypes._MSG_RESP_ECTYPESTATE;
      }
      
      public function readBuf(param1:ByteArray) : void
      {
         var _loc2_:MsgSocket = null;
         _loc2_ = MsgSocket.Instance();
         usSize = _loc2_.readMsgSize(param1);
         usType = _loc2_.readMsgType(param1);
         this.EctypeId = _loc2_.readShort(param1);
         this.GateId = _loc2_.readUnsignChar(param1);
         this.state = _loc2_.readChar(param1);
         _loc2_ = null;
      }
      
      public function writeBuf(param1:ByteArray) : void
      {
         var _loc2_:MsgSocket = MsgSocket.Instance();
         param1.position = 0;
         _loc2_.writeShort(param1,usSize);
         _loc2_.writeShort(param1,usType);
         _loc2_.writeShort(param1,this.EctypeId);
         _loc2_.writeUnsignChar(param1,this.GateId);
         _loc2_.writeChar(param1,this.state);
         _loc2_.PushByte(param1,usSize - param1.position);
         _loc2_ = null;
      }
      
      public function getLength(param1:int = 0) : int
      {
         param1 = 4;
         return param1 + 4;
      }
      
      public function release() : void
      {
      }
   }
}

