package logic.action
{
   ...
   public class GalaxyMapAction extends AbstraceAction
   {
      ...
      public function GalaxyMapAction(param1:Singleton)
      {
         var _loc3_:XML = null;
         var _loc4_:Dirt = null;
         this._dirtArr = new Array();
         super();
         super.ActionName = "Map_GalaxyMap_Action";
      ...
      public static function get instance() : GalaxyMapAction
      {
         if(_instance == null)
         {
            _instance = new GalaxyMapAction(new Singleton());
         }
         return _instance;
      }
      ...
      public function cacheGalaxyMapBgWH() : void
      {
         ...
         {
            this.galaxyMapBgWidth = this.getGalaxyMapBg().BackgroundWH.x;
            this.galayMapBgHeight = this.getGalaxyMapBg().BackgroundWH.y;
         }
      }
      ...
      override public function Init() : void
      {
         ...
         this._curMapSprite.x = -ballGridW * 10;
         this._curMapSprite.y = -ballGridH * 10;
      ...
      private function onClick(param1:MouseEvent) : void
      {
         ...
         if(Boolean(GalaxyManager.instance.getCacheData(_loc4_)) && GalaxyManager.instance.getCacheData(_loc4_).Type != GalaxyType.GT_0)
         {
      ...
      private function RequestMoveHome() : void
      {
         ...
         while(_loc1_ < ScienceSystem.getinstance().Packarr.length)
         {
            if(ScienceSystem.getinstance().Packarr[_loc1_].PropsId == 901)
            {
               ...
               _loc2_.ToGalaxyId = this.curStar.GalaxyMapId;
               _loc2_.ToGalaxyId = this.curStar.GalaxyId;
      ...
      private function onStartMove(param1:MouseEvent) : void
      {
         ...
         {
            param1.target.startDrag();
            this.lastPosX = param1.target.x;
            this.lastPosY = param1.target.y;
            _loc2_ = 0;
      ...
      private function onStopMove(param1:MouseEvent) : void
      {
         ...
         if((param1.target.x - -ballGridW * 10) % ballGridW < 0)
         {
            param1.target.x = -ballGridW * 10 + (param1.target.x - -ballGridW * 10) % ballGridW;
         }
         else if((param1.target.x - -ballGridW * 10) % ballGridW > 0)
         {
            param1.target.x = -ballGridW * 10 - (ballGridW - (param1.target.x - -ballGridW * 10) % ballGridW);
         }
         if((param1.target.y - -ballGridH * 10) % ballGridH < 0)
         {
            param1.target.y = -ballGridH * 10 + (param1.target.y - -ballGridH * 10) % ballGridH;
         }
         else if((param1.target.y - -ballGridH * 10) % ballGridH > 0)
         {
            param1.target.y = -ballGridH * 10 - (ballGridH - (param1.target.y - -ballGridH * 10) % ballGridH);
         }
      }
      ...
      private function searchCacheData(param1:int, param2:int) : int
      {
         ...
         if(_loc4_)
         {
            return _loc7_ + _loc5_ * GalaxyManager.MAX_MAPAREAGRID + _loc6_;
         }
         _loc8_ = 0;
         while(_loc8_ < 3)
         {
      ...
      {
         if(Boolean(this._curStar) && Boolean(this.lastTimeStar))
         {
      ...
      public function GetStarInfo(param1:MouseEvent) : GStar
      {
         ...
         var _loc3_:int = Math.floor(param1.localY / ballGridH);
         var _loc4_:int = _loc3_ + _loc2_ * GalaxyManager.AREAGRIDY * 3;
         if(Boolean(GalaxyManager.instance.getCacheData(_loc4_)) && GalaxyManager.instance.getCacheData(_loc4_).Type != 0)
         {
      ...
      {
         this.mapButtonStatus = (this.mapButtonStatus + 1) % 2;
