package logic.ui
{
   ...
   public class ComposeUI_CommanderChip
   {
      ...
      private function SetChip(param1:int, param2:int) : void
      {
         ...
         var _loc3_:propsInfo = CPropsReader.getInstance().GetPropsInfo(this.SelectedChipInfo.PropsId);
         var _loc4_:int = int(this.GroupIdList.indexOf(_loc3_.GroupId));
         if(_loc4_ >= 0 && _loc4_ != param1)
         {
         ...
         {
            _loc5_ = 0;
            while(_loc5_ < ChipLottery.LockList.length)
            {
               if(ChipLottery.LockList[_loc5_] > param2)
               {
                  --ChipLottery.LockList[_loc5_];
               }
               _loc5_++;
            }
            this.RequestChipList();
