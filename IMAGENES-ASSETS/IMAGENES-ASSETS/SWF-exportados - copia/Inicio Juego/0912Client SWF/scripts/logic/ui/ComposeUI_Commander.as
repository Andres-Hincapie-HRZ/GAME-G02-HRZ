package logic.ui
{
   ...
   public class ComposeUI_Commander
   {
      ...
      private function ShowCommanderValue() : void
      {
         ...
         while(_loc1_ < 12)
         {
            _loc6_ = this.DiamondListMc.getChildByName("tf_" + _loc1_) as TextField;
            if(this.CommanderValueList[_loc1_] > 0 || this.CommanderValueList[_loc1_] != "")
            {
               if(_loc1_ > 3)
               {
                  _loc7_ = int(this.CommanderValueList[_loc1_]) / 10;
                  _loc6_.text = "+" + _loc7_ + "%";
               }
               else
               {
                  _loc6_.text = " + " + this.CommanderValueList[_loc1_].toString();
               }
            }
            else
            {
