package logic.ui
{
   ...
   public class ConstructionUI extends AbstractPopUp
   {
      ...
      private function setLRBtnState() : void
      {
         ...
         if(this._curPage == 0)
         {
            this._left.setBtnDisabled(true);
         }
         if(this._curPage == 0 && (this._curPage + 1) * GameSetting.CONSTRUCTION_CELL_NUMBER >= _loc1_.length)
         {
            this._right.setBtnDisabled(true);
         }
         else if((this._curPage + 1) * GameSetting.CONSTRUCTION_CELL_NUMBER > _loc1_.length)
         {
            this._left.setBtnDisabled(false);
            this._right.setBtnDisabled(true);
         }
         else
         {
            this._right.setBtnDisabled(false);
         }
      }
